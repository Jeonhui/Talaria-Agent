...
# 5. Symlinks the 'talaria' CLI command into a user-facing bin dir
...
echo ""
echo -e "${CYAN}⚕ Talaria Agent Setup${NC}"
echo ""
...
# ============================================================================
# PATH setup — symlink talaria into a user-facing bin dir
# ============================================================================

echo -e "${CYAN}→${NC} Setting up talaria command..."

TALARIA_BIN="$SCRIPT_DIR/venv/bin/talaria"
COMMAND_LINK_DIR="$(get_command_link_dir)"
COMMAND_LINK_DISPLAY_DIR="$(get_command_link_display_dir)"
mkdir -p "$COMMAND_LINK_DIR"
ln -sf "$TALARIA_BIN" "$COMMAND_LINK_DIR/talaria"
echo -e "${GREEN}✓${NC} Symlinked talaria → $COMMAND_LINK_DISPLAY_DIR/talaria"
...
# ============================================================================
# Seed bundled skills into ~/.talaria/skills/
# ============================================================================

TALARIA_SKILLS_DIR="${TALARIA_HOME:-$HOME/.talaria}/skills"
mkdir -p "$TALARIA_SKILLS_DIR"

echo ""
echo "Syncing bundled skills to ~/.talaria/skills/ ..."
if "$SCRIPT_DIR/venv/bin/python" "$SCRIPT_DIR/tools/skills_sync.py" 2>/dev/null; then
    echo -e "${GREEN}✓${NC} Skills synced"
else
    # Fallback: copy if sync script fails (missing deps, etc.)
    if [ -d "$SCRIPT_DIR/skills" ]; then
        cp -rn "$SCRIPT_DIR/skills/"* "$TALARIA_SKILLS_DIR/" 2>/dev/null || true
...
echo "Other commands:"
echo "  talaria status        # Check configuration"
if is_termux; then
    echo "  talaria gateway       # Run gateway in foreground"
else
    echo "  talaria gateway install # Install gateway service (messaging + cron)"
fi
echo "  talaria cron list     # View scheduled jobs"
echo "  talaria doctor        # Diagnose issues"
echo ""
...
if [[ $REPLY =~ ^[Yy]$ ]] || [[ -z $REPLY ]]; then
    ...
    "$SCRIPT_DIR/venv/bin/python" -m talaria_cli.main setup
fi
