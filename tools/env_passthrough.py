...
def _is_talaria_provider_credential(name: str) -> bool:
    """True if ``name`` is a Talaria-managed provider credential (API key,
    token, or similar) per ``_TALARIA_PROVIDER_ENV_BLOCKLIST``.
    ...
    Non-Talaria API keys (TENOR_API_KEY, NOTION_TOKEN, etc.) are NOT
    ...
    try:
        ...
        return False
    return name in _TALARIA_PROVIDER_ENV_BLOCKLIST
...
def register_env_passthrough(var_names: Iterable[str]) -> None:
    ...
    Variables that are Talaria-managed provider credentials (from
    ``_TALARIA_PROVIDER_ENV_BLOCKLIST``) are rejected here to preserve
    the ``execute_code`` sandbox's credential-scrubbing guarantee per
    GHSA-rhgp-j443-p4rf. A skill that needs to talk to a Talaria-managed
    ...
    Non-Talaria third-party API keys (TENOR_API_KEY, NOTION_TOKEN, etc.)
    ...
    for name in var_names:
        ...
        if _is_talaria_provider_credential(name):
            logger.warning(
                "env passthrough: refusing to register Talaria provider "
                "credential %r (blocked by _TALARIA_PROVIDER_ENV_BLOCKLIST). "
