...
`mcp_tool.py`, and `talaria_cli/mcp_config.py`. This module is the ONLY place
...
# ---------------------------------------------------------------------------
# TalariaMCPOAuthProvider — OAuthClientProvider subclass with disk-watch
# ---------------------------------------------------------------------------


def _make_talaria_provider_class() -> Optional[type]:
    ...
    class TalariaMCPOAuthProvider(OAuthClientProvider):
        ...
        def __init__(self, *args: Any, server_name: str = "", **kwargs: Any):
            ...
            self._talaria_server_name = server_name
        ...
        async def _initialize(self) -> None:
            ...
            Paired with :class:`TalariaTokenStorage` persisting an absolute
                ...
                for url in build_protected_resource_metadata_discovery_urls(
                    ...
                    try:
                        ...
                        logger.debug(
                            ...
                            self._talaria_server_name, url, exc,
                        )
                        continue
                ...
                for url in build_oauth_authorization_server_metadata_discovery_urls(
                    ...
                    if asm:
                        ...
                        logger.debug(
                            ...
                            "token_endpoint=%s",
                            self._talaria_server_name, asm.token_endpoint,
                        )
                        break
    ...
    return TalariaMCPOAuthProvider


# Cached at import time. Tested and used by :class:`MCPOAuthManager`.
_TALARIA_PROVIDER_CLS: Optional[type] = _make_talaria_provider_class()
...
class MCPOAuthManager:
    ...
    def _build_provider(
        ...
        Constructs :class:`TalariaMCPOAuthProvider` directly using the helpers
        ...
        Returns None if the MCP SDK's OAuth support is unavailable.
        """
        if _TALARIA_PROVIDER_CLS is None:
            logger.warning(
        ...
        storage = TalariaTokenStorage(server_name)
    ...
    def remove(self, server_name: str) -> None:
        ...
        Called by ``talaria mcp remove <name>`` and (indirectly) by
        ``talaria mcp login <name>`` during forced re-auth.
        """
        with self._entries_lock:
