...
Lets the LLM write a Python script that calls Talaria tools via RPC,
collapsing multi-step tool chains into a single inference turn.

Architecture (two transports):

  **Local backend (UDS):**
  1. Parent generates a `talaria_tools.py` stub module with UDS RPC functions
  ...
  **Remote backends (file-based RPC):**
  1. Parent generates `talaria_tools.py` with file-based RPC stubs
...
# ---------------------------------------------------------------------------
# talaria_tools.py code generator
# ---------------------------------------------------------------------------
...
def generate_talaria_tools_module(enabled_tools: List[str],
                                 transport: str = "uds") -> str:
    """
    Build the source code for the talaria_tools.py stub module.
...
"""Auto-generated Talaria tools RPC stubs."""
...
def _connect():
    ...
    if _sock is None:
        ...
        _sock.connect(os.environ["TALARIA_RPC_SOCKET"])
...
"""Auto-generated Talaria tools RPC stubs (file-based transport)."""
import json, os, shlex, tempfile, time

_RPC_DIR = os.environ.get("TALARIA_RPC_DIR") or os.path.join(tempfile.gettempdir(), "talaria_rpc")
_seq = 0
''' + _COMMON_HELPERS + '''\
...
def _execute_remote(
    ...
    The script and the generated talaria_tools.py module are shipped to
    ...
    sandbox_dir = f"{temp_dir}/talaria_exec_{sandbox_id}"
    ...
    try:
        ...
        tools_src = generate_talaria_tools_module(
            list(sandbox_tools), transport="file",
        )
        _ship_file_to_remote(env, f"{sandbox_dir}/talaria_tools.py", tools_src)
        ...
        env_prefix = (
            f"TALARIA_RPC_DIR={shlex.quote(f'{sandbox_dir}/rpc')} "
            f"PYTHONDONTWRITEBYTECODE=1"
        )
        tz = os.getenv("TALARIA_TIMEZONE", "").strip()
        if tz:
            env_prefix += f" TZ={tz}"
...
def execute_code(
    ...
    # --- Set up temp directory with talaria_tools.py and script.py ---
    tmpdir = tempfile.mkdtemp(prefix="talaria_sandbox_")
    # Use /tmp on macOS to avoid the long /var/folders/... path that pushes
    # Unix domain socket paths past the 104-byte macOS AF_UNIX limit.
    # On Linux, tempfile.gettempdir() already returns /tmp.
    _sock_tmpdir = "/tmp" if sys.platform == "darwin" else tempfile.gettempdir()
    sock_path = os.path.join(_sock_tmpdir, f"talaria_rpc_{uuid.uuid4().hex}.sock")
    ...
    try:
        # Write the auto-generated talaria_tools module
        # sandbox_tools is already the correct set (intersection with session
        # tools, or SANDBOX_ALLOWED_TOOLS as fallback — see lines above).
        tools_src = generate_talaria_tools_module(list(sandbox_tools))
        with open(os.path.join(tmpdir, "talaria_tools.py"), "w") as f:
            f.write(tools_src)
        ...
        for k, v in os.environ.items():
            ...
            if any(k.startswith(p) for p in _SAFE_ENV_PREFIXES):
                child_env[k] = v
        child_env["TALARIA_RPC_SOCKET"] = sock_path
        child_env["PYTHONDONTWRITEBYTECODE"] = "1"
        # Ensure the talaria-agent root is importable in the sandbox so
        # repo-root modules are available to child scripts.  We also prepend
        # the staging tmpdir so ``from talaria_tools import ...`` resolves even
        # when the subprocess CWD is not tmpdir (project mode).
        _talaria_root = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
        _existing_pp = child_env.get("PYTHONPATH", "")
        _pp_parts = [tmpdir, _talaria_root]
        if _existing_pp:
            _pp_parts.append(_existing_pp)
        child_env["PYTHONPATH"] = os.pathsep.join(_pp_parts)
        # Inject user's configured timezone so datetime.now() in sandboxed
        # code reflects the correct wall-clock time.  Only TZ is set —
        # TALARIA_TIMEZONE is an internal Talaria setting and must not leak
        # into child processes.
        _tz_name = os.getenv("TALARIA_TIMEZONE", "").strip()
        if _tz_name:
            child_env["TZ"] = _tz_name
        child_env.pop("TALARIA_TIMEZONE", None)

        # Per-profile HOME isolation: redirect system tool configs into
        # {TALARIA_HOME}/home/ when that directory exists.
        from talaria_constants import get_subprocess_home
        ...
        # but scripts can still read secrets from disk (e.g. open('~/.talaria/.env')).
...
def _get_execution_mode() -> str:
    ...
    Mode semantics:
      ...
      - ``strict``: scripts run in an isolated temp directory with
        ``sys.executable`` (talaria-agent's python). Reproducible and the
...
def build_execute_code_schema(enabled_sandbox_tools: set = None,
    ...
    When tools are disabled via ``talaria tools`` (e.g. web is turned off),
    ...
    # temp-dir staging and talaria-agent's own python.
    if mode == "strict":
        cwd_note = (
            "Scripts run in their own temp dir, not the session's CWD — use absolute paths "
            "(os.path.expanduser('~/.talaria/.env')) or terminal()/read_file() for user files."
        )
    else:
        cwd_note = (
    ...
    description = (
        "Run a Python script that can call Talaria tools programmatically. "
        ...
        f"Available via `from talaria_tools import ...`:\n\n"
        f"{tool_lines}\n\n"
        ...
        "Also available (no import needed — built into talaria_tools):\n"
            ...
            "properties": {
                "code": {
                    ...
                    "description": (
                        ...
                        f"`from talaria_tools import {import_str}` "
