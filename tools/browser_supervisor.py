...
One ``CDPSupervisor`` runs per Talaria ``task_id`` that has a reachable CDP
...
DIALOG_BRIDGE_HOST = "talaria-dialog-bridge.invalid"
...
(() => {
  if (window.__talariaDialogBridgeInstalled) return;
  window.__talariaDialogBridgeInstalled = true;
  const ENDPOINT = "http://talaria-dialog-bridge.invalid/";
