...
# (talaria-* prefixed), and scenario toolsets.
#
...
_SUBAGENT_TOOLSETS = sorted(
    ...
    and not name.startswith("talaria-")
...
def _dump_subagent_timeout_diagnostic(
    *,
    ...
    Captures the child's config, system-prompt / tool-schema sizes, activity
    tracker snapshot, and the worker thread's Python stack at timeout. Written
    to ``~/.talaria/logs/subagent-timeout-<sid>-<ts>.log``.
...
def _resolve_delegation_credentials(cfg: dict, parent_agent) -> dict:
    ...
    if not api_key:
        raise ValueError(
            ...
            f"Set the appropriate environment variable or run 'talaria auth'."
        )
...
def _load_config() -> dict:
    ...
    Checks the runtime config (cli_config.CLI_CONFIG) first, then falls back
    to the persistent config (talaria_cli/config.py load_config()) so that
