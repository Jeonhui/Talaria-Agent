#!/usr/bin/env python3
"""
Skills Hub — Source adapters and hub state management for the Talaria Skills Hub.
...
Used by talaria_cli/skills_hub.py for CLI commands and the /skills slash command.
"""
...
class GitHubSource(SkillSource):
    ...
    def inspect(self, identifier: str) -> Optional[SkillMeta]:
        ...
        if isinstance(metadata, dict):
            talaria_meta = metadata.get("talaria", {})
            if isinstance(talaria_meta, dict):
                tags = talaria_meta.get("tags", [])
        if not tags:
...
class UrlSource(SkillSource):
    ...
    def inspect(self, identifier: str) -> Optional[SkillMeta]:
        ...
        if isinstance(metadata, dict):
            talaria_meta = metadata.get("talaria", {})
            if isinstance(talaria_meta, dict):
                raw_tags = talaria_meta.get("tags", [])
...
class OptionalSkillSource(SkillSource):
    ...
    ~/.talaria/skills/ during setup.  They are discoverable via the Skills Hub
    ...
    def _scan_all(self) -> List[SkillMeta]:
        ...
        for skill_md in sorted(self._optional_dir.rglob("SKILL.md")):
            ...
            if isinstance(meta_block, dict):
                talaria_meta = meta_block.get("talaria", {})
                if isinstance(talaria_meta, dict):
                    tags = talaria_meta.get("tags", [])
...
def _load_talaria_index() -> Optional[dict]:
    ...
    if TALARIA_INDEX_CACHE_FILE.exists():
        try:
            age = time.time() - TALARIA_INDEX_CACHE_FILE.stat().st_mtime
            if age < TALARIA_INDEX_TTL:
                return json.loads(TALARIA_INDEX_CACHE_FILE.read_text())
        except (OSError, json.JSONDecodeError):
            pass
    ...
    try:
        ...
        if resp.status_code != 200:
            logger.debug("Talaria index fetch returned %d", resp.status_code)
            return _load_stale_index_cache()
        data = resp.json()
    except (httpx.HTTPError, json.JSONDecodeError) as e:
        logger.debug("Talaria index fetch failed: %s", e)
        return _load_stale_index_cache()
    ...
    # Cache locally
    try:
        TALARIA_INDEX_CACHE_FILE.parent.mkdir(parents=True, exist_ok=True)
        TALARIA_INDEX_CACHE_FILE.write_text(json.dumps(data))
    except OSError:
        pass
...
class TalariaIndexSource(SkillSource):
    """Skill source backed by the centralized Talaria Skills Index.
    ...
    def _ensure_loaded(self) -> dict:
        if not self._loaded:
            self._index = _load_talaria_index()
            self._loaded = True
    ...
    def fetch(self, identifier: str) -> Optional[SkillBundle]:
        ...
        if resolved:
            ...
            if bundle:
                bundle.source = entry.get("source", "talaria-index")
    ...
    def _to_meta(entry: dict) -> SkillMeta:
        return SkillMeta(
            name=entry.get("name", ""),
            description=entry.get("description", ""),
            source=entry.get("source", "talaria-index"),
            identifier=entry.get("identifier", ""),
...
def create_source_router(auth: Optional[GitHubAuth] = None) -> List[SkillSource]:
    ...
    sources: List[SkillSource] = [
        ...
        TalariaIndexSource(auth=auth), # Centralized index (search + resolved install paths)
...
def parallel_search_sources(
    ...
    *on_source_done* is an optional callback ``(source_id, count) -> None``
    ...
    if source_filter == "all":
        for src in sources:
            if (src.source_id() == "talaria-index"
