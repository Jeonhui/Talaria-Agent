...
from tools.environments.local import _TALARIA_PROVIDER_ENV_BLOCKLIST
...
def _load_talaria_env_vars() -> dict[str, str]:
    """Load ~/.talaria/.env values without failing Docker command execution."""
    try:
        from talaria_cli.config import load_env
...
def find_docker() -> Optional[str]:
    ...
    Resolution order:
    1. ``TALARIA_DOCKER_BINARY`` env var — explicit override (e.g. ``/usr/bin/podman``)
    ...
    override = os.getenv("TALARIA_DOCKER_BINARY")
    if override and os.path.isfile(override) and os.access(override, os.X_OK):
        _docker_executable = override
        logger.info("Using TALARIA_DOCKER_BINARY override: %s", override)
        return override
...
#   SETUID/SETGID - the image entrypoint drops from root to the 'talaria'
...
class DockerEnvironment(BaseEnvironment):
    ...
    def __init__(
        ...
        # (TERMINAL_SANDBOX_DIR, default ~/.talaria/sandboxes/). Non-persistent
        ...
        container_name = f"talaria-{uuid.uuid4().hex[:8]}"
        run_cmd = [
    ...
    def _build_init_env_args(self) -> list[str]:
        ...
        # win over the generic Talaria secret blocklist. Only implicit passthrough
        # keys are filtered.
        forward_keys = explicit_forward_keys | (passthrough_keys - _TALARIA_PROVIDER_ENV_BLOCKLIST)
        talaria_env = _load_talaria_env_vars() if forward_keys else {}
        for key in sorted(forward_keys):
            value = os.getenv(key)
            if value is None:
                value = talaria_env.get(key)
    ...
    def cleanup(self):
        ...
        if self._container_id:
            # Quote interpolated values — _docker_exe comes from the
            # TALARIA_DOCKER_BINARY env var and must not be able to inject
            # shell into this fire-and-forget cleanup command.
            _exe = shlex.quote(self._docker_exe)
            _cid = shlex.quote(self._container_id)
            try:
                # Stop in background so cleanup doesn't block
                stop_cmd = (
                    f"(timeout 60 {_exe} stop {_cid} || "
                    f"{_exe} rm -f {_cid}) >/dev/null 2>&1 &"
                )
            ...
            if not self._persistent:
                ...
                try:
                    subprocess.Popen(
                        f"sleep 3 && {_exe} rm -f {_cid} >/dev/null 2>&1 &",
                        shell=True,
                    )
                except Exception:
                    pass
