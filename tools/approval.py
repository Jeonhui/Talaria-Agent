...
def _fire_approval_hook(hook_name: str, **kwargs) -> None:
    ...
    """
    try:
        from talaria_cli.plugins import invoke_hook
    except Exception:
...
def get_current_session_key(default: str = "default") -> str:
    ...
    return get_session_env("TALARIA_SESSION_KEY", default)

# Sensitive write targets that should trigger approval even when referenced
# via shell expansions like $HOME or $TALARIA_HOME.
_SSH_SENSITIVE_PATH = r'(?:~|\$home|\$\{home\})/\.ssh(?:/|$)'
_TALARIA_ENV_PATH = (
    r'(?:~\/\.talaria/|'
    r'(?:\$home|\$\{home\})/\.talaria/|'
    r'(?:\$talaria_home|\$\{talaria_home\})/)'
    r'\.env\b'
)
...
DANGEROUS_PATTERNS = [
    ...
    (r'\btalaria\s+gateway\s+(stop|restart)\b', "stop/restart talaria gateway (kills running agents)"),
    (r'\btalaria\s+update\b', "talaria update (restarts gateway, kills running agents)"),
    # Gateway protection: never start gateway outside systemd management
    (r'gateway\s+run\b.*(&\s*$|&\s*;|\bdisown\b|\bsetsid\b)', "start gateway outside systemd (use 'systemctl --user restart talaria-gateway')"),
    (r'\bnohup\b.*gateway\s+run\b', "start gateway outside systemd (use 'systemctl --user restart talaria-gateway')"),
    # Self-termination protection: prevent agent from killing its own process
    (r'\b(pkill|killall)\b.*\b(talaria|gateway|cli\.py)\b', "kill talaria/gateway process (self-termination)"),
    # Self-termination via kill + command substitution (pgrep/pidof).
    # The name-based pattern above catches `pkill talaria` but not
    # `kill -9 $(pgrep -f talaria)` because the substitution is opaque
...
def prompt_dangerous_approval(command: str, description: str,
    ...
    os.environ["TALARIA_SPINNER_PAUSE"] = "1"
    try:
        while True:
            print()
        ...
        if "TALARIA_SPINNER_PAUSE" in os.environ:
            del os.environ["TALARIA_SPINNER_PAUSE"]
        print()
        sys.stdout.flush()
...
def check_dangerous_command(command: str, env_type: str,
    ...
    if os.getenv("TALARIA_YOLO_MODE") or is_current_session_yolo_enabled():
        return {"approved": True, "message": None}
    ...
    is_cli = os.getenv("TALARIA_INTERACTIVE")
    is_gateway = os.getenv("TALARIA_GATEWAY_SESSION")

    if not is_cli and not is_gateway:
        # Cron sessions: respect cron_mode config
        if os.getenv("TALARIA_CRON_SESSION"):
    ...
    if is_gateway or os.getenv("TALARIA_EXEC_ASK"):
...
def check_all_command_guards(command: str, env_type: str,
    ...
    if os.getenv("TALARIA_YOLO_MODE") or is_current_session_yolo_enabled() or approval_mode == "off":
        return {"approved": True, "message": None}

    is_cli = os.getenv("TALARIA_INTERACTIVE")
    is_gateway = os.getenv("TALARIA_GATEWAY_SESSION")
    is_ask = os.getenv("TALARIA_EXEC_ASK")
