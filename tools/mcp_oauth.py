...
This module provides the glue:
    - ``TalariaTokenStorage``: persists tokens/client-info to disk so they
    ...
    mcp_servers:
      my_server:
        ...
        oauth:                                  # all fields optional
          ...
          client_name: "My Custom Client"       # default: "Talaria Agent"
"""
...
def _get_token_dir() -> Path:
    ...
    Uses TALARIA_HOME so each profile gets its own OAuth tokens.
    Layout: ``TALARIA_HOME/mcp-tokens/``
    """
    try:
        from talaria_constants import get_talaria_home
        base = Path(get_talaria_home())
    except ImportError:
        base = Path(os.environ.get("TALARIA_HOME", str(Path.home() / ".talaria")))
...
# ---------------------------------------------------------------------------
# TalariaTokenStorage -- persistent token/client-info on disk
# ---------------------------------------------------------------------------


class TalariaTokenStorage:
    """Persist OAuth tokens and client registration to JSON files.

    File layout::

        TALARIA_HOME/mcp-tokens/<server_name>.json         -- tokens
        TALARIA_HOME/mcp-tokens/<server_name>.client.json   -- client info
    """
    ...
    async def get_tokens(self) -> "OAuthToken | None":
        ...
        if data is None:
            return None
        # Talaria records an absolute wall-clock ``expires_at`` alongside the
    ...
    class _Handler(BaseHTTPRequestHandler):
        def do_GET(self) -> None:  # noqa: N802
            ...
            body = (
                ...
                "<p>You can close this tab and return to Talaria.</p></body></html>"
            ) if code else (
...
def _build_client_metadata(cfg: dict) -> "OAuthClientMetadata":
    ...
    client_name = cfg.get("client_name", "Talaria Agent")
    scope = cfg.get("scope")
