...
it is automatically downloaded from GitHub releases to $TALARIA_HOME/bin/tirith.
...
def _get_talaria_home() -> str:
    """Return the Talaria home directory, respecting TALARIA_HOME env var."""
    return str(get_talaria_home())


def _failure_marker_path() -> str:
    """Return the path to the install-failure marker file."""
    return os.path.join(_get_talaria_home(), ".tirith-install-failed")
...
def _talaria_bin_dir() -> str:
    """Return $TALARIA_HOME/bin, creating it if needed."""
    d = os.path.join(_get_talaria_home(), "bin")
...
def _install_tirith(*, log_failures: bool = True) -> tuple[str | None, str]:
    """Download and install tirith to $TALARIA_HOME/bin/tirith.
    ...
    try:
        ...
        dest = os.path.join(_talaria_bin_dir(), "tirith")
        try:
...
def _resolve_tirith_path(configured_path: str) -> str:
    ...
    2. $TALARIA_HOME/bin/tirith (previously auto-installed)
    3. Auto-install from GitHub releases → $TALARIA_HOME/bin/tirith
    ...
    talaria_bin = os.path.join(_talaria_bin_dir(), "tirith")
    if os.path.isfile(talaria_bin) and os.access(talaria_bin, os.X_OK):
        _resolved_path = talaria_bin
        _install_failure_reason = ""
        _clear_install_failed()
        return talaria_bin
