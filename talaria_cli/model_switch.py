...
- ``talaria_cli.providers``        -- canonical provider identity + overlays
- ``talaria_cli.model_normalize``  -- per-provider name formatting
...
def _ensure_direct_aliases() -> None:
    ...
    the module attribute. This keeps `from talaria_cli.model_switch import
...
def switch_model(
    ...
    if explicit_provider:
        ...
        if pdef is None:
            _switch_err = (
                ...
                f"Check 'talaria model' for available providers, or define it "
            ...
            try:
                ...
                if _cfg_issues:
                    _switch_err += "\n\nRun 'talaria doctor' — config issues detected:"
    ...
    # Mirrors the same logic in talaria_cli.runtime_provider.resolve_runtime_provider;
...
def list_authenticated_providers(
    ...
    Uses the curated model lists from talaria_cli/models.py (OPENROUTER_MODELS,
    ...
    def _record_builtin_endpoint(slug: str) -> None:
        ...
        try:
            from talaria_cli.auth import PROVIDER_REGISTRY as _reg
        except Exception:
            return
        pcfg = _reg.get(slug)
        if not pcfg:
    ...
    # --- 1. Check Talaria-mapped providers ---
    for talaria_id, mdev_id in PROVIDER_TO_MODELS_DEV.items():
        ...
        pconfig = PROVIDER_REGISTRY.get(talaria_id)
        # Skip non-API-key auth providers here — they are handled in
        # section 2 (TALARIA_OVERLAYS) with proper auth store checking.
        ...
        if not has_creds:
            try:
                from talaria_cli.auth import _load_auth_store
                store = _load_auth_store()
                if store and talaria_id in store.get("credential_pool", {}):
                    has_creds = True
            except Exception:
                pass
        if not has_creds:
        ...
        # show up in the picker without requiring a Talaria release.
        model_ids = curated.get(talaria_id, [])
        if talaria_id in _MODELS_DEV_PREFERRED:
            model_ids = _merge_with_models_dev(talaria_id, model_ids)
        total = len(model_ids)
    ...
    # while _PROVIDER_MODELS and config.yaml use Talaria IDs ("copilot").
    _mdev_to_talaria = {v: k for k, v in PROVIDER_TO_MODELS_DEV.items()}

    for pid, overlay in TALARIA_OVERLAYS.items():
        if pid.lower() in seen_slugs:
            continue

        # Resolve Talaria slug — e.g. "github-copilot" → "copilot"
        talaria_slug = _mdev_to_talaria.get(pid, pid)
        if talaria_slug.lower() in seen_slugs:
            continue
        ...
        if not has_creds:
            try:
                ...
                if store and (
                    pid in providers_store or talaria_slug in providers_store
                    or pid in pool_store or talaria_slug in pool_store
                ):
                    has_creds = True
        ...
        if not has_creds:
            try:
                ...
                pool = load_pool(talaria_slug)
                if pool.has_credentials():
                    has_creds = True
            except Exception as exc:
                logger.debug("Credential pool check failed for %s: %s", talaria_slug, exc)
        ...
        # configured.
        if not has_creds and talaria_slug == "anthropic":
            try:
                from agent.anthropic_adapter import (
                    read_claude_code_credentials,
                    read_talaria_oauth_credentials,
                )
                talaria_creds = read_talaria_oauth_credentials()
                cc_creds = read_claude_code_credentials()
                if (talaria_creds and talaria_creds.get("accessToken")) or \
        ...
        if talaria_slug in {"copilot", "copilot-acp"}:
            model_ids = provider_model_ids(talaria_slug)
        ...
        # reflects the active region (eu.*, ap.*) not the static us.* list.
            ...
            try:
                ...
                model_ids = _ids if _ids is not None else (curated.get(talaria_slug, []) or curated.get(pid, []))
            except Exception:
                model_ids = curated.get(talaria_slug, []) or curated.get(pid, [])
        else:
            # Use curated list — look up by Talaria slug, fall back to overlay key
            model_ids = curated.get(talaria_slug, []) or curated.get(pid, [])
            # Merge with models.dev for preferred providers (same rationale as above).
            if talaria_slug in _MODELS_DEV_PREFERRED:
                model_ids = _merge_with_models_dev(talaria_slug, model_ids)
        total = len(model_ids)
        top = model_ids[:max_models]

        results.append({
            "slug": talaria_slug,
            "name": get_label(talaria_slug),
            "is_current": talaria_slug == current_provider or pid == current_provider,
            "is_user_defined": False,
            "models": top,
            "total_models": total,
            "source": "talaria",
        ...
        seen_slugs.add(talaria_slug.lower())
        _record_builtin_endpoint(talaria_slug)

    # --- 2b. Cross-check canonical provider list ---
    # Catches providers that are in CANONICAL_PROVIDERS but weren't found
    # in PROVIDER_TO_MODELS_DEV or TALARIA_OVERLAYS (keeps /model in sync
    # with `talaria model`).
    try:
        from talaria_cli.models import CANONICAL_PROVIDERS as _canon_provs
    except ImportError:
        _canon_provs = []
    ...
    if user_providers and isinstance(user_providers, dict):
        for ep_name, ep_cfg in user_providers.items():
            ...
            # ``base_url`` is Talaria's canonical write key (matches
            ...
            # Talaria writes ``models:`` as a dict keyed by model id
            # (see talaria_cli/main.py::_save_custom_provider); older
    ...
    if custom_providers and isinstance(custom_providers, list):
        ...
        for entry in custom_providers:
            ...
            if group_key not in groups:
                ...
                # Talaria's own writer uses; a hyphen variant is accepted
            ...
            # active model. Talaria's own writer (main.py::_save_custom_provider)
            # stores every configured model as a dict under ``models:``;
            # downstream readers (agent/models_dev.py, gateway/run.py,
            # run_agent.py, talaria_cli/config.py) already consume that dict.
