...
Add, remove, or reorder entries here — both `talaria setup` and
`talaria` provider-selection will pick up the change automatically.
"""
...
_TALARIA_USER_AGENT = f"talaria-cli/{_TALARIA_VERSION}"
...
def _codex_curated_models() -> list[str]:
    ...
    This keeps the gateway /model picker in sync with the CLI `talaria model`
    flow without maintaining a separate static list.
    """
    from talaria_cli.codex_models import DEFAULT_CODEX_MODELS, _add_forward_compat_models
...
def get_pricing_for_provider(
    ...
    """Talaria does not fetch live pricing tables."""
    return {}
...
class ProviderEntry(NamedTuple):
    slug: str
    label: str
    tui_desc: str   # detailed description for `talaria model` TUI
...
def get_default_model_for_provider(provider: str) -> str:
    ...
    model a user would be offered first in the ``talaria model`` picker.

    Used as a fallback when the user has configured a provider but never
    selected a model (e.g. ``talaria auth add openai-codex`` without
    ``talaria model``).
    """
...
def list_available_providers() -> list[dict[str, str]]:
    ...
    source of truth shared with ``talaria model``, ``/model``, etc.).
    """
    ...
    for pid in provider_order:
        ...
        has_creds = False
        try:
            from talaria_cli.auth import get_auth_status, has_usable_secret
            if pid == "custom":
...
def _resolve_static_model_alias(
    ...
    try:
        from talaria_cli.model_switch import MODEL_ALIASES
    except Exception:
        return None
...
def normalize_provider(provider: Optional[str]) -> str:
    """Normalize provider aliases to Talaria' canonical provider ids.

    Note: ``"auto"`` passes through unchanged — use
    ``talaria_cli.auth.resolve_provider()`` to resolve it to a concrete
...
def model_supports_fast_mode(model_id: Optional[str]) -> bool:
    """Return whether Talaria should expose the /fast toggle for this model."""
...
def _resolve_copilot_catalog_api_key() -> str:
    ...
    Resolution order:
      ...
      2. ``read_credential_pool("copilot")`` — a token (typically a
         ...
         populated by ``talaria auth add copilot`` and by ``_seed_from_env``
         when the env var is set in ``~/.talaria/.env``.
    ...
    try:
        from talaria_cli.auth import read_credential_pool
        from talaria_cli.copilot_auth import (
...
def provider_model_ids(provider: Optional[str], *, force_refresh: bool = False) -> list[str]:
    ...
    on the platform appear in ``/model`` without a Talaria release.
    """
    normalized = normalize_provider(provider)
    if normalized == "openrouter":
    ...
    if normalized == "openai-codex":
        ...
        # a Talaria release). Falls back to the hardcoded catalog if no token
...
def _lmstudio_request_headers(api_key: Optional[str] = None) -> dict:
    ...
    headers = {"User-Agent": _TALARIA_USER_AGENT}
...
_COPILOT_MODEL_ALIASES = {
    ...
    # Dash-notation fallbacks: Talaria' default Claude IDs elsewhere use
...
def probe_api_models(
    ...
    tried: list[str] = []
    headers: dict[str, str] = {"User-Agent": _TALARIA_USER_AGENT}
    ...
    *,
    ...
    if normalized == "custom":
        ...
        message = (
            ...
            f"Talaria will still save `{requested}`, but the endpoint should expose `/models` for verification."
        )
        if api_mode == "anthropic_messages":
            message += (
        ...
        if catalog_models:
            ...
            return {
                ...
                "message": (
                    ...
                    f"{suggestion_text}"
                    "\n  MiniMax does not expose a /models endpoint, so Talaria cannot verify the model name."
