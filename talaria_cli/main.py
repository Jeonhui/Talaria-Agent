...
def _exec_in_container(container_info: dict, cli_args: list):
    ...
    Args:
        container_info: dict with backend, container_name, exec_user, talaria_bin
        cli_args: the original CLI arguments (everything after 'talaria')
    """

    backend = container_info["backend"]
    container_name = container_info["container_name"]
    exec_user = container_info["exec_user"]
    talaria_bin = container_info["talaria_bin"]
        ...
        if sudo_path:
            ...
            if probe2.returncode != 0:
                print(
                    ...
                    f"  }}];\n"
                    f"\n"
                    f"Or run: sudo talaria {' '.join(cli_args)}",
                    file=sys.stderr,
                )
                sys.exit(1)
        else:
            print(
                f"Error: container '{container_name}' not found via {backend}.\n"
                f"The container may be running under root. Try: sudo talaria {' '.join(cli_args)}",
                file=sys.stderr,
            )
            sys.exit(1)
...
def _model_flow_openrouter(config, current_model=""):
    ...
    from talaria_cli.config import get_env_value, save_env_value
    ...
    from talaria_cli.models import model_ids, get_pricing_for_provider
...
def _model_flow_ai_gateway(config, current_model=""):
    ...
    from talaria_cli.models import ai_gateway_model_ids, get_pricing_for_provider
...
def _model_flow_nous(config, current_model="", args=None):
    ...
    from talaria_cli.nous_subscription import prompt_enable_tool_gateway
    ...
    if free_tier and not model_ids:
        ...
        if unavailable_models:
            from talaria_cli.auth import DEFAULT_NOUS_PORTAL_URL
...
def _model_flow_azure_foundry(config, current_model=""):
    ...
    * Manual entry when every probe fails (private endpoints, etc.).
    ...
    """
    from talaria_cli.auth import _save_model_choice, deactivate_provider  # noqa: F401
    from talaria_cli.config import get_env_value, save_env_value, load_config, save_config
    from talaria_cli import azure_detect
    import getpass
    ...
    print("Anthropic-style API endpoints.  Talaria will probe your")
...
def _model_flow_copilot(config, current_model=""):
    ...
    from talaria_cli.config import save_env_value, load_config, save_config
    from talaria_cli.models import (
        fetch_api_models,
    ...
    if not api_key:
        ...
        if choice == "1":
            try:
                from talaria_cli.copilot_auth import copilot_device_code_login
            ...
            # Validate token type
            try:
                from talaria_cli.copilot_auth import validate_copilot_token
...
def _model_flow_copilot_acp(config, current_model=""):
    ...
    print("  GitHub Copilot ACP delegates Talaria turns to `copilot --acp`.")
    print("  Talaria currently starts its own ACP subprocess for each request.")
    print("  Talaria uses your selected model as a hint for the Copilot ACP session.")
    ...
    try:
        ...
        print(f"  ⚠ {exc}")
        print(
            "  Set TALARIA_COPILOT_ACP_COMMAND or COPILOT_CLI_PATH if Copilot CLI is installed elsewhere."
        )
        return
...
def _model_flow_bedrock_api_key(config, region, current_model=""):
    ...
    if selected:
        ...
        # Save the API key env var name so talaria knows where to find it
...
def _coalesce_session_name_args(argv: list) -> list:
    ...
    When a user types ``talaria -c Pokemon Agent Dev`` without quoting the
