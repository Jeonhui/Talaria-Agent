"""
Backup and import commands for talaria CLI.

`talaria backup` creates a zip archive of the entire ~/.talaria/ directory
(excluding the talaria-agent repo and transient files).

`talaria import` restores from a backup zip, overlaying onto the current
TALARIA_HOME root.
"""
...
_EXCLUDED_DIRS = {
    "talaria-agent",     # the codebase repo — re-clone instead
...
def _should_exclude(rel_path: Path) -> bool:
    """Return True if *rel_path* (relative to talaria root) should be skipped."""
...
def run_backup(args) -> None:
    """Create a zip backup of the Talaria home directory."""
    talaria_root = get_default_talaria_root()

    if not talaria_root.is_dir():
        print(f"Error: Talaria home directory not found at {talaria_root}")
        sys.exit(1)
    ...
    if args.output:
        ...
        if out_path.is_dir():
            ...
            out_path = out_path / f"talaria-backup-{stamp}.zip"
    else:
        stamp = datetime.now().strftime("%Y-%m-%d-%H%M%S")
        out_path = Path.home() / f"talaria-backup-{stamp}.zip"
    ...
    # Collect files
    print(f"Scanning {display_talaria_home()} ...")
    files_to_add: list[tuple[Path, Path]] = []  # (absolute, relative)
    skipped_dirs = set()

    for dirpath, dirnames, filenames in os.walk(talaria_root, followlinks=False):
        dp = Path(dirpath)
        rel_dir = dp.relative_to(talaria_root)
        ...
        for fname in filenames:
            fpath = dp / fname
            rel = fpath.relative_to(talaria_root)

            if _should_exclude(rel):
                continue

            # Skip the output zip itself if it happens to be inside talaria root
            try:
    ...
    print(f"\nRestore with: talaria import {out_path.name}")
...
def _validate_backup_zip(zf: zipfile.ZipFile) -> tuple[bool, str]:
    """Check that a zip looks like a Talaria backup.
    ...
    # Look for telltale files that a talaria home would have
    ...
    if not found:
        return False, (
            "zip does not appear to be a Talaria backup "
...
def _detect_prefix(zf: zipfile.ZipFile) -> str:
    ...
    Some tools zip as `.talaria/config.yaml` instead of `config.yaml`.
    ...
    if len(first_parts) == 1:
        ...
        # Only strip if it looks like a talaria dir name
        if prefix in (".talaria", "talaria"):
            return prefix + "/"
...
def run_import(args) -> None:
    ...
    with zipfile.ZipFile(zip_path, "r") as zf:
        ...
        print(f"Target: {display_talaria_home()}")

        if prefix:
            print(f"Detected archive prefix: {prefix!r} (will be stripped)")

        # Check for existing installation
        has_config = (talaria_root / "config.yaml").exists()
        has_env = (talaria_root / ".env").exists()

        if (has_config or has_env) and not args.force:
            print()
            print("Warning: Target directory already has Talaria configuration.")
        ...
        talaria_root.mkdir(parents=True, exist_ok=True)
        ...
        for member in members:
            ...
            target = talaria_root / rel

            # Security: reject absolute paths and traversals
            try:
                target.resolve().relative_to(talaria_root.resolve())
            except ValueError:
        ...
        profiles_dir = talaria_root / "profiles"
        restored_profiles = []
        ...
        # Guidance
        print()
        if not (talaria_root / "talaria-agent").is_dir():
            print("Note: The talaria-agent codebase was not included in the backup.")
            print("  If this is a fresh install, run: talaria update")

        if restored_profiles:
            gw_profiles = [n for n, _ in restored_profiles]
            print("\nTo re-enable gateway services for profiles:")
            for pname in gw_profiles:
                print(f"  talaria -p {pname} gateway install")

        print("Done. Your Talaria configuration has been restored.")


# ---------------------------------------------------------------------------
# Quick state snapshots (used by /snapshot slash command and talaria backup --quick)
# ---------------------------------------------------------------------------

# Critical state files to include in quick snapshots (relative to TALARIA_HOME).
...
# — `talaria update` snapshots this set before pulling so approved-user lists
...
def _quick_snapshot_root(talaria_home: Optional[Path] = None) -> Path:
    home = talaria_home or get_talaria_home()
    return home / _QUICK_SNAPSHOTS_DIR


def create_quick_snapshot(
    label: Optional[str] = None,
    talaria_home: Optional[Path] = None,
) -> Optional[str]:
...
def list_quick_snapshots(
    ...
    root = _quick_snapshot_root(talaria_home)
    if not root.exists():
        return []
...
def prune_quick_snapshots(
    ...
    return _prune_quick_snapshots(_quick_snapshot_root(talaria_home), keep=keep)


def run_quick_backup(args) -> None:
    """CLI entry point for talaria backup --quick."""
    label = getattr(args, "label", None)
    snap_id = create_quick_snapshot(label=label)
    if snap_id:
        print(f"State snapshot created: {snap_id}")
        snaps = list_quick_snapshots()
        print(f"  {len(snaps)} snapshot(s) stored in {display_talaria_home()}/state-snapshots/")
...
def _write_full_zip_backup(out_path: Path, talaria_root: Path) -> Optional[Path]:
    """Write a full zip snapshot of ``talaria_root`` to ``out_path``.
...
def _pre_update_backup_dir(talaria_home: Optional[Path] = None) -> Path:
    home = talaria_home or get_talaria_home()
...
def create_pre_update_backup(
    ...
    """Create a full zip backup of TALARIA_HOME under ``backups/``.

    Mirrors :func:`run_backup` (same exclusion rules, same SQLite safe-copy)
    but writes to ``<TALARIA_HOME>/backups/pre-update-<timestamp>.zip`` and
    auto-prunes old pre-update backups.

    Returns the path to the created zip, or ``None`` if no files were
    found or the backup could not be created.  Never raises — the caller
    (``talaria update``) should continue even if the backup fails.
    """
    talaria_root = talaria_home or get_default_talaria_root()
    if not talaria_root.is_dir():
        return None

    backup_dir = _pre_update_backup_dir(talaria_root)
    try:
    ...
    result = _write_full_zip_backup(out_path, talaria_root)
    if result is None:
        return None
...
# ---------------------------------------------------------------------------
# Pre-migration auto-backup (used by `talaria claw migrate`)
# ---------------------------------------------------------------------------
...
def create_pre_migration_backup(
    ...
    """Create a full zip backup of TALARIA_HOME under ``backups/`` before a
    ``talaria claw migrate`` apply.

    Shares implementation with :func:`create_pre_update_backup` via
    ``_write_full_zip_backup`` — same exclusions, same SQLite safe-copy,
    restorable with ``talaria import <archive>``.  Writes to
    ``<TALARIA_HOME>/backups/pre-migration-<timestamp>.zip`` and auto-prunes
    ...
    # Reuses the shared backups/ directory so `talaria import` and the
