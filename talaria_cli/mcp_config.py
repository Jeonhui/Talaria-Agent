...
def _prompt(question: str, *, password: bool = False, default: str = "") -> str:
    from talaria_cli.cli_output import prompt as _shared_prompt
...
def cmd_mcp_add(args):
            ...
            if auth_type == "header" or not auth_type:
                ...
                if existing_key:
                    ...
                    if api_key:
                        ...
                        _success(f"Saved to {display_talaria_home()}/.env as {env_key}")
    ...
    print()
    _success(f"Saved '{name}' to {display_talaria_home()}/config.yaml ({tool_count}/{total} tools enabled)")
...
def cmd_mcp_remove(args):
    ...
    # earlier `talaria mcp test` in the same session) is evicted too.
    try:
        from tools.mcp_oauth_manager import get_manager
...
# ─── talaria mcp configure ────────────────────────────────────────────────────

def cmd_mcp_configure(args):
    """Reconfigure which tools are enabled for an existing MCP server."""
    import sys as _sys
    if not _sys.stdin.isatty():
        print("Error: 'talaria mcp configure' requires an interactive terminal.", file=_sys.stderr)
        _sys.exit(1)
    name = args.name
    servers = _get_mcp_servers()
...
def mcp_command(args):
    """Main dispatcher for ``talaria mcp`` subcommands."""
