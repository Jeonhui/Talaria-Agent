"""
Unified tool configuration for Talaria Agent.

`talaria tools` and `talaria setup tools` both enter this module.
Select a platform → toggle toolsets on/off → for newly enabled tools
that need API keys, run through provider-aware configuration.

Saves per-platform tool configuration to ~/.talaria/config.yaml under
...
# They're still in _TALARIA_CORE_TOOLS (available at runtime if enabled),
# but the setup checklist won't pre-select them for first-time users.
_DEFAULT_OFF_TOOLSETS = {"browser", "discord", "discord_admin"}

# Platform-scoped toolsets: only appear in the `talaria tools` checklist for
...
def _get_effective_configurable_toolsets():
    """Return the intentionally small Talaria toolset checklist."""
    return list(CONFIGURABLE_TOOLSETS)


def _get_plugin_toolset_keys() -> set:
    """Talaria does not expose plugin toolsets in the default checklist."""
    return set()

# Platform display config — derived from the canonical registry so every
# module shares the same data.  Kept as dict-of-dicts for backward
# compatibility with existing ``PLATFORMS[key]["label"]`` access patterns.
from talaria_cli.platforms import PLATFORMS as _PLATFORMS_REGISTRY
    ...
    if post_setup_key in ("agent_browser", "browserbase"):
        ...
        if not node_modules.exists() and npm_bin:
            ...
            if result.returncode == 0:
                ...
                _print_warning(f"    npm install failed - run manually: cd {display_talaria_home()}/talaria-agent && npm install")
                if result.stderr:
                    _print_info(f"      {result.stderr.strip()[:200]}")
        elif not node_modules.exists():
            _print_warning("    Node.js not found - browser tools require: npm install (in talaria-agent directory)")
            return
        ...
        if _running_in_docker():
            ...
            _print_info(
                "      docker pull ghcr.io/nousresearch/talaria-agent:latest"
            )
            return
        ...
        # Run the full `talaria auth spotify` flow — if the user has no
        # client_id yet, this drops them into the interactive wizard
        # (opens the Spotify dashboard, prompts for client_id, persists
        # to ~/.talaria/.env), then continues straight into PKCE. If they
        # already have an app, it skips the wizard and just does OAuth.
        from types import SimpleNamespace
        try:
            from talaria_cli.auth import login_spotify_command
        except Exception as exc:
        ...
        try:
            ...
            # toolset enable; they can retry with `talaria auth spotify`.
            _print_warning(f"    Spotify login did not complete: {exc}")
            _print_info("    Run later: talaria auth spotify")
        except Exception as exc:
        ...
        try:
            from talaria_cli.plugins_cmd import _get_enabled_set, _save_enabled_set
            ...
            _print_info("    Run manually: talaria plugins enable observability/langfuse")
        _print_info("    Restart Talaria for tracing to take effect.")
        _print_info("    Verify: talaria plugins list")
...
def _get_platform_tools(
    ...
    *,
    ...
    # "talaria-cli" (which include all _TALARIA_CORE_TOOLS) cause disabled
    ...
    if has_explicit_config:
        ...
        # (e.g. "talaria-cli") to individual tool names and reverse-mapping.
        all_tool_names = set()
        ...
        # even on their own platform (e.g. `discord` + `discord` stays OFF).
    ...
    # otherwise saving via `talaria tools` (which flips has_explicit_config
    # to True) silently drops them.
    _plat_info = PLATFORMS.get(platform)
    _default_ts = _plat_info["default_toolset"] if _plat_info else f"talaria-{platform}"
    ...
    skip |= {k for k in TOOLSETS if k.startswith("talaria-")}
    ...
    # shipped as a bundled plugin but user must opt in via `talaria tools`
    # so we don't ship 7 Spotify tool schemas to users who don't use it).
    # A plugin toolset is "known" for a platform once `talaria tools`
    ...
    if plugin_ts_keys:
        ...
        for pts in plugin_ts_keys:
            if pts in toolset_names:
                ...
                # New plugin not yet seen by talaria tools — default enabled
                enabled_toolsets.add(pts)
...
def _save_platform_tools(config: dict, platform: str, enabled_toolset_keys: Set[str]):
    ...
    # Also exclude platform default toolsets (talaria-cli, talaria-telegram, etc.)
    ...
    # Opening `talaria tools` is the user's opt-in to reconfigure tools, so treat
...
def tools_command(args=None, first_install: bool = False, config: dict = None):
    """Entry point for `talaria tools` and `talaria setup tools`.
    ...
    print()
    from talaria_constants import display_talaria_home
    print(color(f"  Tool configuration saved to {display_talaria_home()}/config.yaml", Colors.DIM))
    print(color("  Changes take effect on next 'talaria' or gateway restart.", Colors.DIM))
    print()
