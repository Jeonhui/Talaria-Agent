"""`talaria checkpoints` CLI subcommand.

Gives users direct visibility and control over the filesystem checkpoint
store at ``~/.talaria/checkpoints/``.  Actions:

    talaria checkpoints               # same as `status`
    talaria checkpoints status        # total size, project count, breakdown
    talaria checkpoints list          # per-project checkpoint counts + workdir
    talaria checkpoints prune [opts]  # force a sweep (ignores the 24h marker)
    talaria checkpoints clear [-f]    # nuke the entire base (asks first)
    talaria checkpoints clear-legacy  # delete just the legacy-* archives

Examples::

    talaria checkpoints
    talaria checkpoints prune --retention-days 3
    talaria checkpoints clear -f
...
def cmd_status(args: argparse.Namespace) -> int:
    ...
    if legacy:
        ...
        print()
        print("Clear with: talaria checkpoints clear-legacy")
    return 0
...
def register_cli(parser: argparse.ArgumentParser) -> None:
    """Wire subcommands onto the ``talaria checkpoints`` parser."""
    parser.set_defaults(func=cmd_status)  # bare `talaria checkpoints` → status
