"""talaria webhook — manage dynamic webhook subscriptions from the CLI.

Usage:
    talaria webhook subscribe <name> [options]
    talaria webhook list
    talaria webhook remove <name>
    talaria webhook test <name> [--payload '{"key": "value"}']

Subscriptions persist to ~/.talaria/webhook_subscriptions.json and are
...
def _setup_hint() -> str:
  ...
  Then start the gateway: talaria gateway run
"""
...
def webhook_command(args):
    """Entry point for 'talaria webhook' subcommand."""
    sub = getattr(args, "webhook_action", None)

    if not sub:
        print("Usage: talaria webhook {subscribe|list|remove|test}")
        print("Run 'talaria webhook --help' for details.")
        return
...
def _cmd_subscribe(args):
    ...
    print(f"  The gateway must be running to receive events (talaria gateway run).\n")
...
def _cmd_list(args):
    ...
    if not subs:
        ...
        print("  Create one with: talaria webhook subscribe <name>")
        return
...
def _cmd_test(args):
    ...
    try:
        ...
        print(f"  Error: {e}")
        print("  Is the gateway running? (talaria gateway run)")
