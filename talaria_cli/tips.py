...
TIPS = [
    ...
    "/title \"my project\" names your session — resume it later with /resume or talaria -c.",
    ...
    "Ctrl+Z suspends Talaria to the background — run fg in your shell to resume.",
    ...
    # --- CLI Flags ---
    "talaria -c resumes your most recent CLI session. talaria -c \"project name\" resumes by title.",
    "talaria -w creates an isolated git worktree — perfect for parallel agent workflows.",
    "talaria -w -q \"Fix issue #42\" combines worktree isolation with a one-shot query.",
    "talaria chat -t web,terminal enables only specific toolsets for a focused session.",
    "talaria chat -s github-pr-workflow preloads a skill at launch.",
    "talaria chat -q \"query\" runs a single non-interactive query and exits.",
    "talaria chat --max-turns 200 overrides the default 90-iteration limit per turn.",
    "talaria chat --checkpoints enables filesystem snapshots before every destructive file change.",
    "talaria --yolo bypasses all dangerous command approval prompts for the entire session.",
    "talaria chat --source telegram tags the session for filtering in talaria sessions list.",
    "talaria -p work chat runs under a specific profile without changing your default.",

    # --- CLI Subcommands ---
    "talaria doctor --fix diagnoses and auto-repairs config and dependency issues.",
    "talaria dump outputs a compact setup summary — great for bug reports.",
    "talaria config set KEY VALUE auto-routes secrets to .env and everything else to config.yaml.",
    "talaria config edit opens config.yaml in your default editor.",
    "talaria config check scans for missing or stale configuration options.",
    "talaria sessions browse opens an interactive session picker with search.",
    "talaria sessions stats shows session counts by platform and database size.",
    "talaria sessions prune --older-than 30 cleans up old sessions.",
    "talaria skills search react --source skills-sh searches the skills.sh public directory.",
    "talaria skills check scans installed hub skills for upstream updates.",
    "talaria skills tap add myorg/skills-repo adds a custom GitHub skill source.",
    "talaria skills snapshot export setup.json exports your skill configuration for backup or sharing.",
    "talaria mcp add lets you register MCP servers with your own URL or stdio command.",
    "talaria mcp serve runs Talaria itself as an MCP server for other agents.",
    "talaria auth add lets you add multiple API keys for credential pool rotation.",
    "talaria completion bash >> ~/.bashrc enables tab completion for all commands and profiles.",
    "talaria logs -f follows agent.log in real time. --level WARNING --since 1h filters output.",
    "talaria backup creates a zip backup of your entire Talaria home directory.",
    "talaria profile create coder creates an isolated profile that becomes its own command.",
    ...
    "Set worktree: true in config.yaml to always create a git worktree (same as talaria -w).",
    ...
    "TALARIA_TIMEZONE overrides the server timezone with any IANA timezone string.",
    ...
    # --- Tools & Capabilities ---
    "execute_code runs Python scripts that call Talaria tools programmatically — results stay out of context.",
    ...
    "Profile names become shell commands — 'talaria profile create coder' creates the 'coder' command.",
    "talaria profile export coder -o backup.tar.gz creates a portable profile archive.",
    ...
    "When exiting, Talaria prints a resume command with session ID and stats.",
    "talaria sessions export backup.jsonl exports all sessions for backup or analysis.",
    "talaria -r SESSION_ID resumes any specific past session by its ID.",
    ...
    "The plan skill saves markdown plans under .talaria/plans/ in the active workspace.",
    ...
    "Context files (.talaria.md, AGENTS.md) are security-scanned for prompt injection before loading.",
    ...
    "SOUL.md at ~/.talaria/SOUL.md is the agent's primary identity — customize it to shape behavior.",
    "Talaria loads project context from .talaria.md, AGENTS.md, CLAUDE.md, or .cursorrules (first match).",
    ...
    "MCP servers auto-generate toolsets at runtime — talaria tools can toggle them per platform.",
    ...
    "Checkpoints use shadow repos in ~/.talaria/checkpoints/ — your project's .git is never touched.",

    # --- Batch & Data ---
    "batch_runner.py processes hundreds of prompts in parallel for training data generation.",
    "talaria chat -Q enables quiet mode for programmatic use — suppresses banner and spinner.",
    ...
    "talaria acp runs Talaria as an ACP server for VS Code, Zed, and JetBrains integration.",
    ...
    "Cron scripts live in ~/.talaria/scripts/ and run before the agent — perfect for data collection pipelines.",
    "prefill_messages_file in config.yaml injects few-shot examples into every API call, never saved to history.",
    "SOUL.md completely replaces the agent's default identity — rewrite it to make Talaria your own.",
    "SOUL.md is auto-seeded with a default personality on first run. Edit ~/.talaria/SOUL.md to customize.",
    ...
    "Before a gateway session reset, Talaria auto-flushes important facts to memory in the background.",
    ...
    "The skills audit log at ~/.talaria/skills/.hub/audit.log tracks every install and removal operation.",
