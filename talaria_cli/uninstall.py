"""
Talaria Agent Uninstaller.

Provides options for:
- Full uninstall: Remove everything including configs and data
- Keep data: Remove code but keep ~/.talaria/ (configs, sessions, logs)
"""
...
def remove_path_from_shell_configs():
    """Remove Talaria PATH entries from shell configuration files."""
    ...
    for config_path in configs:
        try:
            ...
            # Remove lines containing talaria-agent or talaria PATH entries
            new_lines = []
            skip_next = False
            
            for line in content.split('\n'):
                # Skip the "# Talaria Agent" comment and following line
                if '# Talaria Agent' in line or '# talaria-agent' in line:
                    skip_next = True
                    continue
                if skip_next and ('talaria' in line.lower() and 'PATH' in line):
                    skip_next = False
                    continue
                skip_next = False
                
                # Remove any PATH line containing talaria
                if 'talaria' in line.lower() and ('PATH=' in line or 'path=' in line.lower()):
                    continue
...
def remove_wrapper_script():
    """Remove the talaria wrapper script if it exists."""
    wrapper_paths = [
        Path.home() / ".local" / "bin" / "talaria",
        Path("/usr/local/bin/talaria"),
    ]
    ...
    for wrapper in wrapper_paths:
        if wrapper.exists():
            try:
                # Check if it's our wrapper (contains talaria_cli reference)
                content = wrapper.read_text()
                if 'talaria_cli' in content or 'talaria-agent' in content:
                    wrapper.unlink()
...
def uninstall_gateway_service():
    ...
    - All platforms: standalone ``talaria gateway run`` processes
    ...
    # 3. macOS: uninstall launchd plist
        ...
        try:
            from talaria_cli.gateway import get_launchd_plist_path
...
def _is_default_talaria_home(talaria_home: Path) -> bool:
    """Return True when ``talaria_home`` points at the default (non-profile) root."""
    try:
        from talaria_constants import get_default_talaria_root
        return talaria_home.resolve() == get_default_talaria_root().resolve()
    except Exception:
        return False
...
def _uninstall_profile(profile) -> None:
    ...
    remove its alias wrapper, and wipe its TALARIA_HOME directory.

    We shell out to ``talaria -p <name> gateway stop|uninstall`` because
    service names, unit paths, and plist paths are all derived from the
    current TALARIA_HOME and can't be easily switched in-process.
    """
    import sys as _sys
    name = profile.name
    ...
    #    Use `python -m talaria_cli.main` so we don't depend on a `talaria`
    #    wrapper that may be half-removed mid-uninstall.
    talaria_invocation = [_sys.executable, "-m", "talaria_cli.main", "--profile", name]
    for subcmd in ("stop", "uninstall"):
        try:
            subprocess.run(
                talaria_invocation + ["gateway", subcmd],
                capture_output=True,
                text=True,
                timeout=60,
                check=False,
    ...
    # 3. Wipe the profile's TALARIA_HOME directory.
    try:
...
def run_uninstall(args):
    ...
    Options:
    - Full uninstall: removes code + ~/.talaria/ (configs, data, logs)
    - Keep data: removes code but keeps ~/.talaria/ for future reinstall
    """
    project_root = get_project_root()
    talaria_home = get_talaria_home()

    # Detect named profiles when uninstalling from the default root —
    # offer to clean them up too instead of leaving zombie TALARIA_HOMEs
    # and systemd units behind.
    is_default_profile = _is_default_talaria_home(talaria_home)
    ...
    print(f"  Config:  {talaria_home / 'config.yaml'}")
    print(f"  Secrets: {talaria_home / '.env'}")
    print(f"  Data:    {talaria_home / 'cron/'}, {talaria_home / 'sessions/'}, {talaria_home / 'logs/'}")
    print()
    ...
    # their alias wrappers, and wiping their TALARIA_HOME dirs. Otherwise
    ...
    print()
    if full_uninstall:
        print(color("⚠️  WARNING: This will permanently delete ALL Talaria data!", Colors.RED, Colors.BOLD))
        ...
        print("This will remove the Talaria code but keep your configuration and data.")
    ...
    log_info("Removing talaria command...")
    ...
    try:
        if project_root.exists():
            # If the install is inside ~/.talaria/, just remove the talaria-agent subdir
            if talaria_home in project_root.parents or project_root.parent == talaria_home:
    ...
    # 5. Optionally remove ~/.talaria/ data directory (and named profiles)
    if full_uninstall:
        # 5a. Stop and remove each named profile's gateway service and
        #     alias wrapper. The profile TALARIA_HOME dirs live under
        ...
        try:
            if talaria_home.exists():
                shutil.rmtree(talaria_home)
                log_success(f"Removed {talaria_home}")
        except Exception as e:
            log_warn(f"Could not fully remove {talaria_home}: {e}")
            log_info("You may need to manually remove it")
    else:
        log_info(f"Keeping configuration and data in {talaria_home}")
    ...
    if not full_uninstall:
        ...
        print(color("  curl -fsSL https://raw.githubusercontent.com/NousResearch/talaria-agent/main/scripts/install.sh | bash", Colors.DIM))
        print()
