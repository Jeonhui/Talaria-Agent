"""
Gateway subcommand for talaria CLI.

Handles: talaria gateway [run|start|stop|restart|status|install|uninstall|setup]
"""
        ...
        for scope_args in [["systemctl", "--user"], ["systemctl"]]:
            try:
                result = subprocess.run(
                    scope_args + ["list-units", "talaria-gateway*",
...
def _scan_gateway_pids(exclude_pids: set[int], all_profiles: bool = False) -> list[int]:
    ...
    pids: list[int] = []
    patterns = [
        "talaria_cli.main gateway",
        "talaria_cli.main --profile",
        "talaria_cli.main -p",
        "talaria_cli/main.py gateway",
        "talaria_cli/main.py --profile",
        "talaria_cli/main.py -p",
        "talaria gateway",
        "gateway/run.py",
    ]
    current_home = str(get_talaria_home().resolve())
    ...
    def _matches_current_profile(command: str) -> bool:
        if current_profile_name:
            return (
                ...
                or f"TALARIA_HOME={current_home}" in command
            )

        if "--profile " in command or " -p " in command:
            return False
        if "TALARIA_HOME=" in command and f"TALARIA_HOME={current_home}" not in command:
            return False
        return True
...
def find_gateway_pids(exclude_pids: set | None = None, all_profiles: bool = False) -> list:
    ...
    Args:
        ...
        all_profiles: When ``True``, return gateway PIDs across **all**
            profiles (the pre-7923 global behaviour).  ``talaria update``
            needs this because a code update affects every profile.
            When ``False`` (default), only PIDs belonging to the current
            Talaria profile are returned.
    """
...
def _wait_for_systemd_service_restart(
    *,
    ...
    print(
        ...
        f"  Check status: {'sudo ' if system else ''}talaria gateway status\n"
...
def get_gateway_runtime_snapshot(system: bool = False) -> GatewayRuntimeSnapshot:
    ...
    from talaria_constants import is_container
...
def _print_gateway_process_mismatch(snapshot: GatewayRuntimeSnapshot) -> None:
    ...
    print("  This is usually a manual foreground/tmux/nohup run, so `talaria gateway`")
...
def stop_profile_gateway() -> bool:
    """Stop only the gateway for the current profile (TALARIA_HOME-scoped).
...
from talaria_constants import is_container, is_termux, is_wsl
...
_SERVICE_BASE = "talaria-gateway"
SERVICE_DESCRIPTION = "Talaria Agent Gateway - Messaging Platform Integration"


def _profile_suffix() -> str:
    """Derive a service-name suffix from the current TALARIA_HOME.

    Returns ``""`` for the default root, the profile name for
    ``<root>/profiles/<name>``, or a short hash for any other path.
    Works correctly in Docker (TALARIA_HOME=/opt/data) and standard deployments.
    """
    import hashlib
    import re
    ...
    try:
        ...
        pass
    # Fallback: short hash for arbitrary TALARIA_HOME paths
    return hashlib.sha256(str(home).encode()).hexdigest()[:8]


def _profile_arg(talaria_home: str | None = None) -> str:
    """Return ``--profile <name>`` only when TALARIA_HOME is a named profile.

    For ``~/.talaria/profiles/<name>``, returns ``"--profile <name>"``.
    For the default profile or hash-based custom paths, returns the empty string.

    Args:
        talaria_home: Optional explicit TALARIA_HOME path. Defaults to the current
            ``get_talaria_home()`` value. Should be passed when generating a
...
def get_service_name() -> str:
    """Derive a systemd service name scoped to this TALARIA_HOME.

    Default ``~/.talaria`` returns ``talaria-gateway`` (backward compatible).
    Profile ``~/.talaria/profiles/coder`` returns ``talaria-gateway-coder``.
    Any other TALARIA_HOME appends a short hash for uniqueness.
    """
...
# Legacy service names from older Talaria installs that predate the
# talaria-gateway rename. Kept as an explicit allowlist (NOT a glob) so
# profile units (talaria-gateway-*.service) and unrelated third-party
# "talaria" units are never matched.
_LEGACY_SERVICE_NAMES: tuple[str, ...] = ("talaria.service",)
...
def _find_legacy_talaria_units() -> list[tuple[str, Path, bool]]:
    """Return ``[(unit_name, unit_path, is_system)]`` for legacy Talaria gateway units.

    Detects unit files installed by older Talaria versions that used a
    different service name (e.g. ``talaria.service`` before the rename to
    ``talaria-gateway.service``). When both a legacy unit and the current
    ``talaria-gateway.service`` are active, they fight over the same bot
    ...
    * Explicit allowlist of legacy names (no globbing). Profile units such
      as ``talaria-gateway-coder.service`` and unrelated third-party
      ``talaria-*`` services are never matched.
    * ExecStart content check — only flag units that invoke our gateway
      entrypoint. A user-created ``talaria.service`` running an unrelated
...
def has_legacy_talaria_units() -> bool:
    """Return True when any legacy Talaria gateway unit files exist."""
    return bool(_find_legacy_talaria_units())


def print_legacy_unit_warning() -> None:
    """Warn about legacy Talaria gateway unit files if any are installed.

    Idempotent: prints nothing when no legacy units are detected. Safe to
    call from any status/install/setup path.
    """
    legacy = _find_legacy_talaria_units()
    if not legacy:
        return
    print_warning("Legacy Talaria gateway unit(s) detected from an older install:")
    for name, path, is_system in legacy:
        scope = "system" if is_system else "user"
        print_info(f"    {path}  ({scope} scope)")
    print_info("  These run alongside the current talaria-gateway service and")
    print_info("  cause SIGTERM flap loops — both try to use the same bot token.")
    print_info("  Remove them with:")
    print_info("    talaria gateway migrate-legacy")


def remove_legacy_talaria_units(
    interactive: bool = True,
    dry_run: bool = False,
) -> tuple[int, list[Path]]:
    """Stop, disable, and remove legacy Talaria gateway unit files.

    Iterates over whatever ``_find_legacy_talaria_units()`` returns — which is
    ...
    legacy = _find_legacy_talaria_units()
    if not legacy:
        print("No legacy Talaria gateway units found.")
        return 0, []
    ...
    print()
    print("Legacy Talaria gateway unit(s) found:")
    for name, path, is_system in legacy:
        scope = "system" if is_system else "user"
    ...
    if interactive and not prompt_yes_no("Remove these legacy units?", True):
        print("Skipped. Run again with: talaria gateway migrate-legacy")
        return 0, [p for _, p, _ in legacy]
    ...
    if system_units:
        if os.geteuid() != 0:
            ...
            print_info("  Re-run with: sudo talaria gateway migrate-legacy")
...
def print_systemd_scope_conflict_warning() -> None:
    ...
    print_info("    talaria gateway uninstall")
    print_info("    sudo talaria gateway uninstall --system")
...
def install_linux_gateway_from_setup(force: bool = False) -> tuple[str | None, bool]:
    ...
    if scope == "system":
        ...
        if os.geteuid() != 0:
            print_warning("  System service install requires sudo, so Talaria can't create it from this user session.")
            if run_as_user:
                print_info(f"  After setup, run: sudo talaria gateway install --system --run-as-user {run_as_user}")
            else:
                print_info("  After setup, run: sudo talaria gateway install --system --run-as-user <your-user>")
            print_info("  Then start it with: sudo talaria gateway start --system")
            return scope, False
...
def _launchd_user_home() -> Path:
    ...
    Profile-mode Talaria often sets ``HOME`` to a profile-scoped directory, but
...
def get_launchd_plist_path() -> Path:
    ...
    Default ``~/.talaria`` → ``ai.talaria.gateway.plist`` (backward compatible).
    Profile ``~/.talaria/profiles/coder`` → ``ai.talaria.gateway-coder.plist``.
    """
    suffix = _profile_suffix()
    name = f"ai.talaria.gateway-{suffix}" if suffix else "ai.talaria.gateway"
...
def _remap_path_for_user(path: str, target_home_dir: str) -> str:
    ...
    to *target_home_dir*; otherwise the path is returned unchanged.
      ...
      /root/.talaria/talaria-agent  -> /home/alice/.talaria/talaria-agent
      /opt/talaria                 -> /opt/talaria  (kept as-is)
...
def _talaria_home_for_target_user(target_home_dir: str) -> str:
    """Remap the current TALARIA_HOME to the equivalent under a target user's home.

    When installing a system service via sudo, get_talaria_home() resolves to
    root's home.  This translates it to the target user's equivalent path:
      /root/.talaria                    → /home/alice/.talaria
      /root/.talaria/profiles/coder     → /home/alice/.talaria/profiles/coder
      /opt/custom-talaria               → /opt/custom-talaria  (kept as-is)
    """
    current_talaria = get_talaria_home().resolve()
    current_default = (Path.home() / ".talaria").resolve()
    target_default = Path(target_home_dir) / ".talaria"

    # Default ~/.talaria → remap to target user's default
    if current_talaria == current_default:
        return str(target_default)

    # Profile or subdir of ~/.talaria → preserve the relative structure
    try:
        relative = current_talaria.relative_to(current_default)
        return str(target_default / relative)
    except ValueError:
        # Completely custom path (not under ~/.talaria) — keep as-is
        return str(current_talaria)
...
def generate_systemd_unit(system: bool = False, run_as_user: str | None = None) -> str:
    ...
    if system:
        ...
        talaria_home = _talaria_home_for_target_user(home_dir)
        profile_arg = _profile_arg(talaria_home)
...
User={username}
Group={group_name}
ExecStart={python_path} -m talaria_cli.main{f" {profile_arg}" if profile_arg else ""} gateway run --replace
...
Environment="TALARIA_HOME={talaria_home}"
Restart=on-failure
RestartSec=30
...
"""
    ...
    talaria_home = str(get_talaria_home().resolve())
    profile_arg = _profile_arg(talaria_home)
...
def refresh_systemd_unit_if_needed(system: bool = False) -> bool:
    ...
    print(f"↻ Updated gateway {_service_scope_label(system)} service definition to match the current Talaria install")
    return True
...
def systemd_install(force: bool = False, system: bool = False, run_as_user: str | None = None):
    ...
    # Offer to remove legacy units (talaria.service from pre-rename installs)
    # before installing the new talaria-gateway.service. If both remain, they
    ...
    if has_legacy_talaria_units():
        print()
        print_legacy_unit_warning()
        print()
        if prompt_yes_no("Remove the legacy unit(s) before installing?", True):
            remove_legacy_talaria_units(interactive=False)
            print()
    ...
    print()
    print("Next steps:")
    print(f"  {'sudo ' if system else ''}talaria gateway start{scope_flag}              # Start the service")
    print(f"  {'sudo ' if system else ''}talaria gateway status{scope_flag}             # Check status")
...
def systemd_restart(system: bool = False):
    ...
    if pid is not None and _request_gateway_self_restart(pid):
        ...
        # unit immediately so `talaria gateway restart` behaves idempotently.
        _run_systemctl(
            ["reset-failed", svc],
            system=system,
            check=False,
...
def systemd_status(deep: bool = False, system: bool = False, full: bool = False):
    ...
    if not unit_path.exists():
        ...
        print(f"  Run: {'sudo ' if system else ''}talaria gateway install{scope_flag}")
        return
    ...
    if not systemd_unit_is_current(system=system):
        ...
        print(f"  Run: {'sudo ' if system else ''}talaria gateway restart{scope_flag}  # auto-refreshes the unit")
        print()
    ...
    if status == "active":
        ...
        print(f"  Run: {'sudo ' if system else ''}talaria gateway start{scope_flag}")
    ...
    if active_state == "activating" and sub_state == "auto-restart":
        ...
        print(f"  Run: systemctl {'--user ' if not system else ''}reset-failed {get_service_name()} && {'sudo ' if system else ''}talaria gateway start{scope_flag}")
...
def get_launchd_label() -> str:
    ...
    suffix = _profile_suffix()
    return f"ai.talaria.gateway-{suffix}" if suffix else "ai.talaria.gateway"
...
def generate_launchd_plist() -> str:
    ...
    prog_args = [
        ...
        "<string>-m</string>",
        "<string>talaria_cli.main</string>",
    ]
    if profile_arg:
...
<dict>
    ...
    <dict>
        ...
        <key>TALARIA_HOME</key>
        <string>{talaria_home}</string>
    </dict>
...
def refresh_launchd_plist_if_needed() -> bool:
    ...
    print("↻ Updated gateway launchd service definition to match the current Talaria install")
    return True
...
def launchd_install(force: bool = False):
    ...
    print()
    print("Next steps:")
    print("  talaria gateway status             # Check status")
...
def launchd_stop():
    ...
    # `talaria gateway start` re-bootstraps when it detects the job is unloaded.
    try:
...
def launchd_status(deep: bool = False):
    ...
    if launchd_plist_is_current():
        print("✓ Service definition matches the current Talaria install")
    else:
        print("⚠ Service definition is stale relative to the current Talaria install")
        print("  Run: talaria gateway start")
    ...
    if deep:
        log_file = get_talaria_home() / "logs" / "gateway.log"
        if log_file.exists():
            print()
            print("Recent logs:")
...
def _all_platforms() -> list[dict]:
    ...
    ``talaria setup gateway`` without needing the gateway to be running.
    ...
    # User-installed platform plugins under ~/.talaria/plugins/ still require
        ...
        if var.get("is_allowlist"):
            ...
            if value:
                ...
                access_choices = [
                    ...
                    "Use DM pairing (unknown users request access, you approve with 'talaria pairing approve')",
                ...
                if access_idx == 0:
                    ...
                    print_info("  Approve with: talaria pairing approve <platform> <code>")
                else:
                    print_info("  Skipped — configure later with 'talaria gateway setup'")
            continue
...
def _builtin_setup_fn(key: str):
    ...
    Late-bound to avoid a circular import with ``talaria_cli.setup`` (which
    imports from this module for the remaining bespoke flows).
    """
    from talaria_cli import setup as _s
    return {
...
def _configure_platform(platform: dict) -> None:
    ...
    is needed here. User-installed platform plugins under ~/.talaria/plugins/
    ...
    if required:
        print_info(f"  Set these env vars in ~/.talaria/.env: {', '.join(required)}")
    else:
            ...
            if prompt_yes_no("  Restart the gateway to pick up changes?", True):
                try:
                    if supports_systemd_services():
                        ...
                        print_info("Start manually: talaria gateway")
                except UserSystemdUnavailableError as e:
            ...
            if supports_systemd_services() or is_macos():
                ...
                if prompt_yes_no(f"  Install the gateway as a {platform_name} service?{wsl_note} (runs in background, starts on boot)", True):
                    try:
                        ...
                        print_info("  You can try manually: talaria gateway install")
                else:
                    print_info("  You can install later: talaria gateway install")
                    if supports_systemd_services():
                        print_info("  Or as a boot-time service: sudo talaria gateway install --system")
                    print_info("  Or run in foreground:  talaria gateway run")
            elif is_wsl():
                print_info("  WSL detected but systemd is not running.")
                print_info("  Run in foreground: talaria gateway run")
                print_info("  For persistence:   tmux new -s talaria 'talaria gateway run'")
                ...
                if is_termux():
                    ...
                    print_info("  Run in foreground: talaria gateway run")
                    print_info(f"  Or start it manually in the background (best effort): nohup talaria gateway run >{_dhh()}/logs/gateway.log 2>&1 &")
                else:
                    print_info("  Service install not supported on this platform.")
                    print_info("  Run in foreground: talaria gateway run")
    else:
        print()
        print_info("No platforms configured. Run 'talaria gateway setup' when ready.")
...
def _gateway_command_inner(args):
    ...
    if subcmd == "install":
        ...
        if is_termux():
            ...
            print("Run manually: talaria gateway")
            sys.exit(1)
        if supports_systemd_services():
            if is_wsl():
                print_warning("WSL detected — systemd services may not survive WSL restarts.")
                print_info("  Consider running in foreground instead: talaria gateway run")
                print_info("  Or use tmux/screen for persistence: tmux new -s talaria 'talaria gateway run'")
                print()
            ...
            print()
            print("  talaria gateway run                              # direct foreground")
            print("  tmux new -s talaria 'talaria gateway run'         # persistent via tmux")
            print("  nohup talaria gateway run > ~/.talaria/logs/gateway.log 2>&1 &  # background")
            sys.exit(1)
        elif is_container():
            ...
            print()
            print("To run the gateway: talaria gateway run")
            sys.exit(0)
        else:
            print("Service installation not supported on this platform.")
            print("Run manually: talaria gateway run")
            sys.exit(1)
        ...
        if is_termux():
            ...
            print("Stop manual runs with: talaria gateway stop")
            sys.exit(1)
        if supports_systemd_services():
        ...
        if supports_systemd_services():
            ...
            print()
            print("Or run the gateway directly: talaria gateway run")
            sys.exit(0)
        else:
            print("Not supported on this platform.")
            sys.exit(1)
        ...
        if not service_available:
            ...
            if service_configured:
                ...
                print("  Fix the service, then retry: talaria gateway start")
                sys.exit(1)
        ...
        if supports_systemd_services() and (get_systemd_unit_path(system=False).exists() or get_systemd_unit_path(system=True).exists()):
            ...
            if pids:
                ...
                if is_termux():
                    ...
                    print("  talaria gateway install")
                    print("  sudo talaria gateway install --system")
            else:
                ...
                print()
                print("To start:")
                print("  talaria gateway run      # Run in foreground")
                if is_termux():
                    print("  nohup talaria gateway run > ~/.talaria/logs/gateway.log 2>&1 &  # Best-effort background start")
                elif is_wsl():
                    print("  tmux new -s talaria 'talaria gateway run'         # persistent via tmux")
                    print("  nohup talaria gateway run > ~/.talaria/logs/gateway.log 2>&1 &  # background")
                else:
                    print("  talaria gateway install  # Install as user service")
                    print("  sudo talaria gateway install --system  # Install as boot-time system service")

    elif subcmd == "migrate-legacy":
        # Stop, disable, and remove legacy Talaria gateway unit files from
        # pre-rename installs (e.g. talaria.service). Profile units and
        # unrelated third-party services are never touched.
        ...
        if not supports_systemd_services() and not is_macos():
            ...
            return
        remove_legacy_talaria_units(interactive=not yes, dry_run=dry_run)