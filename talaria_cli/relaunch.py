...
from talaria_cli._parser import (
...
def _extract_inherited_flags(argv: Sequence[str]) -> list[str]:
    """Pull out flags that should carry over into a self-relaunched talaria."""
    flags: list[str] = []
    i = 0
    while i < len(argv):
        arg = argv[i]
...
def resolve_talaria_bin() -> Optional[str]:
    """Find the talaria entry point.

    Priority:
      1. ``sys.argv[0]`` if it resolves to a real executable.
      2. ``shutil.which("talaria")`` on PATH.
      3. ``None`` → caller should fall back to ``python -m talaria_cli.main``.
    """
    argv0 = sys.argv[0]
...
def build_relaunch_argv(
    ...
    *,
    ...
    """Construct an argv list for replacing the current process with talaria.
    ...
    Args:
        ...
        original_argv: The original argv to scan for flags (defaults to
            ``sys.argv[1:]``).
    """
    bin_path = resolve_talaria_bin()

    if bin_path:
        argv = [bin_path]
    else:
        argv = [sys.executable, "-m", "talaria_cli.main"]
...
def relaunch(
    ...
    *,
    ...
    """Replace the current process with a fresh talaria invocation."""
