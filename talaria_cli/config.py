"""
Configuration management for Talaria Agent.

Config files are stored in ~/.talaria/ for easy access:
- ~/.talaria/config.yaml  - All settings (model, toolsets, terminal, etc.)
- ~/.talaria/.env         - API keys and secrets

This module provides:
- talaria config          - Show current configuration
- talaria config edit     - Open config in editor
- talaria config set      - Set a specific value
- talaria config wizard   - Re-run setup wizard
"""
...
def get_managed_system() -> Optional[str]:
    ...
    raw = os.getenv("TALARIA_MANAGED", "").strip()
    if raw:
    ...
    managed_marker = get_talaria_home() / ".managed"
...
def is_managed() -> bool:
    """Check if Talaria is running in package-manager-managed mode.

    Two signals: the TALARIA_MANAGED env var (set by the systemd service),
    or a .managed marker file in TALARIA_HOME (set by the NixOS activation
...
def get_managed_update_command() -> Optional[str]:
    ...
    if managed_system == "Homebrew":
        return "brew upgrade talaria-agent"
...
def recommended_update_command() -> str:
    ...
    return get_managed_update_command() or "talaria update"


def format_managed_message(action: str = "modify this Talaria installation") -> str:
    """Build a user-facing error for managed installs."""
    managed_system = get_managed_system() or "a package manager"
    raw = os.getenv("TALARIA_MANAGED", "").strip().lower()

    if managed_system == "NixOS":
        env_hint = "true" if raw in _MANAGED_TRUE_VALUES else raw or "true"
        return (
            f"Cannot {action}: this Talaria installation is managed by NixOS "
            f"(TALARIA_MANAGED={env_hint}).\n"
            "Edit services.talaria-agent.settings in your configuration.nix and run:\n"
    ...
    if managed_system == "Homebrew":
        ...
        return (
            f"Cannot {action}: this Talaria installation is managed by Homebrew "
            f"(TALARIA_MANAGED={env_hint}).\n"
            "Use:\n"
            "  brew upgrade talaria-agent"
        )

    return (
        f"Cannot {action}: this Talaria installation is managed by {managed_system}.\n"
        "Use your package manager to upgrade or reinstall Talaria."
    )
...
def get_container_exec_info() -> Optional[dict]:
    """Read container mode metadata from TALARIA_HOME/.container-mode.

    Returns a dict with keys: backend, container_name, exec_user, talaria_bin
    or None if container mode is not active, we're already inside the
    container, or TALARIA_DEV=1 is set.

    The .container-mode file is written by the NixOS activation script when
    container.enable = true. It tells the host CLI to exec into the container
    instead of running locally.
    """
    if os.environ.get("TALARIA_DEV") == "1":
        return None
    ...
    container_mode_file = get_talaria_home() / ".container-mode"
    ...
    container_name = info.get("container_name", "talaria-agent")
    exec_user = info.get("exec_user", "talaria")
    talaria_bin = info.get("talaria_bin", "/data/current-package/bin/talaria")

    return {
        "backend": backend,
        "container_name": container_name,
        "exec_user": exec_user,
        "talaria_bin": talaria_bin,
    }
...
def _secure_dir(path):
    ...
    permissions (0750) so interactive users in the talaria group can
    share state with the gateway service.

    The mode can be overridden via the TALARIA_HOME_MODE environment variable
    (e.g. TALARIA_HOME_MODE=0701) for deployments where a web server (nginx,
    caddy, etc.) needs to traverse TALARIA_HOME to reach a served subdirectory.
    ...
    if is_managed():
        return
    try:
        mode_str = os.environ.get("TALARIA_HOME_MODE", "").strip()
...
def _is_container() -> bool:
    ...
    When Talaria runs in a container with volume-mounted config files, forcing
    ...
    """
    # Explicit opt-out
    if os.environ.get("TALARIA_CONTAINER") or os.environ.get("TALARIA_SKIP_CHMOD"):
        return True
...
def _secure_file(path):
    ...
    permissions.  Set TALARIA_SKIP_CHMOD=1 to force-skip on other systems.
    """
...
def ensure_talaria_home():
    """Ensure ~/.talaria directory structure exists with secure permissions.
    ...
    if is_managed():
        ...
        try:
            _ensure_talaria_home_managed(home)
        finally:
            os.umask(old_umask)
    else:
...
def _ensure_talaria_home_managed(home: Path):
    """Managed-mode variant: verify dirs exist (activation creates them), seed SOUL.md."""
    if not home.is_dir():
        raise RuntimeError(
            f"TALARIA_HOME {home} does not exist. "
...
DEFAULT_CONFIG = {
    ...
    "agent": {
        ...
        # the Talaria-level retry loop that wraps the whole call.  Lower
    ...
    "terminal": {
        ...
        # skipped. When empty, Talaria auto-sources ``~/.profile``,
        # ``~/.bash_profile``, and ``~/.bashrc`` (in that order) if the
        # snapshot shell is bash (this is the ``auto_source_bashrc``
        # behaviour — disable with that key if you want strict login-only
        # semantics).
        "shell_init_files": [],
        # When true (default), Talaria sources the user's shell rc files
        ...
        # docker_env lets you specify exact key-value pairs — useful when Talaria
        ...
        #  "/home/user/.talaria/cache/documents:/output"]
        ...
        # bundled Talaria image, which drops to the `talaria` user via gosu).
    ...
    "browser": {
        ...
        "camofox": {
            # When true, Talaria sends a stable profile-scoped userId to Camofox
    ...
    "checkpoints": {
        ...
        # ~/.talaria/checkpoints/ (one per cd'd working directory). Field
        # reports put the typical offender at 1000+ repos / ~12 GB. When
        # auto_prune is on, talaria sweeps at startup (at most once per
        # min_interval_hours) and deletes:
        #   * orphan repos: TALARIA_WORKDIR no longer exists on disk
    ...
    # single read_file page exceeds these limits, Talaria truncates the
    ...
    "display": {
        ...
        # e.g. `model · 68% · ~/projects/talaria`. Per-platform overrides go under
    ...
    # a plugin in plugins/context_engine/<name>/ or ~/.talaria/plugins/.
    "context": {
        "engine": "compressor",
    },
    ...
    "memory": {
        ...
        # Add user-installed plugins under ~/.talaria/plugins/memory/<name>/
    ...
    # always goes to ~/.talaria/skills/.
    "skills": {
        "external_dirs": [],   # e.g. ["~/.agents/skills", "/shared/team-skills"]
        # Substitute ${TALARIA_SKILL_DIR} and ${TALARIA_SESSION_ID} in SKILL.md
    ...
    # cron daemon.
    #
    # See `talaria curator status` for the last run summary.
    "curator": {
        "enabled": True,
        ...
        # to use Talaria' main auxiliary client resolution.
        "auxiliary": {
            "provider": None,
            "model": None,
        },
    ...
    # stored approval from ~/.talaria/shell-hooks-allowlist.json.
    # See `website/docs/user-guide/features/hooks.md` for schema + examples.
    "hooks": {},

    # Auto-accept shell-hook registrations without a TTY prompt.  Also
    # toggleable per-invocation via --accept-hooks or TALARIA_ACCEPT_HOOKS=1.
    ...
    "cron": {
        ...
        # Also overridable via TALARIA_CRON_MAX_PARALLEL env var.
    ...
    "code_execution": {
        ...
        #     talaria-agent's own python (sys.executable). Maximum isolation
    ...
    # Logging — controls file logging to ~/.talaria/logs/.
    ...
    # Session storage — controls automatic cleanup of ~/.talaria/state.db.
    ...
    "sessions": {
        ...
        # default of ``talaria sessions prune``.
        "retention_days": 90,
    ...
    # ``talaria update`` behaviour.
    "updates": {
        # Run a full ``talaria backup``-style zip of TALARIA_HOME before every
        # ``talaria update``.  Backups land in ``<TALARIA_HOME>/backups/`` and
        # can be restored with ``talaria import <path>``.  Off by default —
        # on large TALARIA_HOME directories the zip can add minutes to every
...
OPTIONAL_ENV_VARS = {
    # ── Providers (Talaria supports: claude, gpt, codex, mimo, local, custom) ──
    "LM_API_KEY": {
    ...
    "API_SERVER_MODEL_NAME": {
        "description": "Model name advertised on /v1/models. Defaults to the profile name (or 'talaria-agent' for the default profile). Useful for multi-user setups with OpenWebUI.",
    ...
    "GATEWAY_PROXY_URL": {
        "description": "URL of a remote Talaria API server to forward messages to (proxy mode). When set, the gateway handles platform I/O only — all agent work is delegated to the remote server. Use for Docker E2EE containers that relay to a host agent. Also configurable via gateway.proxy_url in config.yaml.",
        "prompt": "Remote Talaria API server URL (e.g. http://192.168.1.100:8642)",
        "url": None,
        "password": False,
        "category": "messaging",
        "advanced": True,
    ...
    "GATEWAY_PROXY_KEY": {
        "description": "Bearer token for authenticating with the remote Talaria API server (proxy mode). Must match the API_SERVER_KEY on the remote host.",
    ...
    "TALARIA_EPHEMERAL_SYSTEM_PROMPT": {
...
def get_missing_skill_config_vars() -> List[Dict[str, Any]]:
    ...
    Scans all enabled skills for ``metadata.talaria.config`` entries, then checks
...
def _normalize_custom_provider_entry(
    ...
    *,
    ...
    if isinstance(models, dict) and models:
        ...
        # Hand-edited configs (and older Talaria versions) write ``models`` as
...
def get_custom_provider_context_length(
    ...
    used by:
      ...
      * ``talaria_cli.model_switch.resolve_display_context_length`` (``/model`` confirmation display)
...
def validate_config_structure(config: Optional[Dict[str, Any]] = None) -> List["ConfigIssue"]:
    ...
    if config is None:
        try:
            ...
            return [ConfigIssue("error", "Could not load config.yaml", "Run 'talaria setup' to create a valid config")]
    ...
    if cp and not model_cfg:
        issues.append(ConfigIssue(
            "warning",
            "custom_providers defined but no 'model' section — Talaria won't know which provider to use",
            "Add a model section:\n"
            "  model:\n"
            "    provider: custom\n"
...
def print_config_warnings(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    lines.append("  \033[2mRun 'talaria doctor' for fix suggestions.\033[0m")
    sys.stderr.write("\n".join(lines) + "\n\n")
...
def warn_deprecated_cwd_env_vars(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    if lines:
        hint_path = os.environ.get("TALARIA_HOME", "~/.talaria")
...
def migrate_config(interactive: bool = True, quiet: bool = False) -> Dict[str, Any]:
    ...
    if current_ver < 4:
        ...
        if "tool_progress" not in display:
            old_enabled = get_env_value("TALARIA_TOOL_PROGRESS")
            old_mode = get_env_value("TALARIA_TOOL_PROGRESS_MODE")
            if old_enabled and old_enabled.lower() in ("false", "0", "no"):
                display["tool_progress"] = "off"
                results["config_added"].append("display.tool_progress=off (from TALARIA_TOOL_PROGRESS=false)")
            elif old_mode and old_mode.lower() in ("new", "all"):
                display["tool_progress"] = old_mode.lower()
                results["config_added"].append(f"display.tool_progress={old_mode.lower()} (from TALARIA_TOOL_PROGRESS_MODE)")
            else:
    ...
    if current_ver < 5:
        ...
        if "timezone" not in config:
            old_tz = os.getenv("TALARIA_TIMEZONE", "")
            if old_tz and old_tz.strip():
                config["timezone"] = old_tz.strip()
                results["config_added"].append(f"timezone={old_tz.strip()} (from TALARIA_TIMEZONE)")
            else:
                config["timezone"] = ""
    ...
    if current_ver < 21:
        ...
        if "enabled" not in plugins_cfg:
            ...
            # Scan ``$TALARIA_HOME/plugins/`` for currently installed user plugins.
            grandfathered: List[str] = []
            try:
                user_plugins_dir = get_talaria_home() / "plugins"
            ...
            if not quiet:
                if grandfathered:
                    ...
                    print(
                        ...
                        "Use `talaria plugins enable <name>` to activate."
                    )
    ...
    if new_var_names and interactive and not quiet:
        ...
        if new_and_unset:
            ...
            if answer in ("y", "yes"):
                ...
                print("  Set later with: talaria config set <key> <value>")
    ...
    # metadata.talaria.config in their SKILL.md frontmatter.
...
def cfg_get(cfg: Optional[Dict[str, Any]], *keys: str, default: Any = None) -> Any:
    ...
    ubiquitous ``cfg_path = _talaria_home / "config.yaml"`` local variable
...
def read_raw_config() -> Dict[str, Any]:
    """Read ~/.talaria/config.yaml as-is, without merging defaults or migrating.
...
def load_config() -> Dict[str, Any]:
    """Load configuration from ~/.talaria/config.yaml.
    ...
    (which change ``TALARIA_HOME`` and therefore ``get_config_path()``)
    don't collide.
    """
    ensure_talaria_home()
    config_path = get_config_path()
...
def save_config(config: Dict[str, Any]):
    """Save configuration to ~/.talaria/config.yaml."""
    if is_managed():
...
def load_env() -> Dict[str, str]:
    """Load environment variables from ~/.talaria/.env.
...
def _sanitize_env_lines(lines: list) -> list:
    ...
    split on real Talaria env var names, avoiding false positives from values
...
def sanitize_env_file() -> int:
    """Read, sanitize, and rewrite ~/.talaria/.env in place.
...
def save_env_value(key: str, value: str):
    """Save or update a value in ~/.talaria/.env."""
    if is_managed():
...
def remove_env_value(key: str) -> bool:
    """Remove a key from ~/.talaria/.env and os.environ.
...
def reload_env() -> int:
    """Re-read ~/.talaria/.env into os.environ. Returns count of vars updated.

    Adds/updates vars that changed and removes vars that were deleted from
    the .env file (but only vars known to Talaria — OPTIONAL_ENV_VARS and
    ...
    for key, value in env_vars.items():
        if os.environ.get(key) != value:
            os.environ[key] = value
            count += 1
    # Remove known Talaria vars that are no longer in .env
...
def get_env_value(key: str) -> Optional[str]:
    """Get a value from ~/.talaria/.env or environment."""
...
def show_config():
    ...
    print(color("  talaria config edit     # Edit config file", Colors.DIM))
    print(color("  talaria config set <key> <value>", Colors.DIM))
    print(color("  talaria setup           # Run setup wizard", Colors.DIM))
    print()
...
def config_command(args):
    ...
    if subcmd is None or subcmd == "show":
        ...
        if not key or value is None:
            print("Usage: talaria config set <key> <value>")
            print()
            print("Examples:")
            print("  talaria config set model anthropic/claude-sonnet-4")
            print("  talaria config set terminal.backend docker")
            print("  talaria config set ANTHROPIC_API_KEY sk-ant-...")
            sys.exit(1)
        ...
        if missing_config:
            ...
            print("    Run 'talaria config migrate' to add them")
        ...
        print("  talaria config           Show current configuration")
        print("  talaria config edit      Open config in editor")
        print("  talaria config set <key> <value>   Set a config value")
        print("  talaria config check     Check for missing/outdated config")
        print("  talaria config migrate   Update config with new options")
        print("  talaria config path      Show config file path")
        print("  talaria config env-path  Show .env file path")
        sys.exit(1)
