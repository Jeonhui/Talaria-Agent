"""Shell completion script generation for talaria CLI.
...
def generate_bash(parser: argparse.ArgumentParser) -> str:
    ...
    for cmd in sorted(tree["subcommands"]):
        ...
        if cmd == "profile" and info["subcommands"]:
            ...
            cases.append(
                ...
                f"                    COMPREPLY=($(compgen -W \"$(_talaria_profiles)\" -- \"$cur\"))\n"
                f"                    return\n"
                f"                    ;;\n"
                f"            esac\n"
                f"            ;;"
    ...
    return f"""# Talaria Agent bash completion
# Add to ~/.bashrc:
...
def generate_zsh(parser: argparse.ArgumentParser) -> str:
    ...
    return f"""#compdef talaria
# Talaria Agent zsh completion
# Add to ~/.zshrc:
...
_talaria() {{
    ...
    _arguments -C \\
        ...
        '(-p --profile){{-p,--profile}}[Profile name]:profile:_talaria_profiles' \\
        '1:command:->commands' \\
        '*::arg:->args'
...
{top_cmds_str}
            ...
            _describe 'talaria command' subcmds
            ;;
        args)
            case ${{line[1]}} in
{sub_cases_str}
...
def generate_fish(parser: argparse.ArgumentParser) -> str:
    ...
    lines: list[str] = [
        "# Talaria Agent fish completion",
        "# Add to your config:",
        "#   talaria completion fish | source",
        "",
        "# Helper: list available profiles",
        "function __talaria_profiles",
        "    echo default",
        "    if test -d $HOME/.talaria/profiles",
        "        ls $HOME/.talaria/profiles 2>/dev/null",
        "    end",
        "end",
        "",
        "# Disable file completion by default",
        "complete -c talaria -f",
        "",
        "# Complete profile names after -p / --profile",
        "complete -c talaria -f -s p -l profile"
        " -d 'Profile name' -xa '(__talaria_profiles)'",
        "",
        ...
        if cmd == "profile":
            for action in sorted(profile_name_actions):
                lines.append(
                    ...
                    f"-a '(__talaria_profiles)' -d 'Profile name'"
                )
