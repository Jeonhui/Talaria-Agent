"""
Profile management for multiple isolated Talaria instances.

Each profile is a fully independent TALARIA_HOME directory with its own
config.yaml, .env, memory, sessions, skills, gateway, cron, and logs.
Profiles live under ``~/.talaria/profiles/<name>/`` by default.

The "default" profile is ``~/.talaria`` itself — backward compatible,
zero migration needed.

Usage::

    talaria profile create coder          # fresh profile + bundled skills
    talaria profile create coder --clone  # also copy config, .env, SOUL.md
    talaria profile create coder --clone-all  # full copy of source profile
    coder chat                           # use via wrapper alias
    talaria -p coder chat                 # or via flag
    talaria profile use coder             # set as sticky default
    talaria profile delete coder          # remove profile + alias + service
"""
...
_PROFILE_DIRS = [
    ...
    # See talaria_constants.get_subprocess_home() and issue #4426.
    "home",
]
...
def _clone_all_copytree_ignore(source_dir: Path):
    ...
    ``~/.talaria`` contains ``profiles/<name>/`` for sibling named profiles.
...
# Directories/files to exclude when exporting the default (~/.talaria) profile.
# The default profile contains infrastructure (repo checkout, worktrees, DBs,
# caches, binaries) that named profiles don't have.  We exclude those so the
# export is a portable, reasonable-size archive of actual profile data.
_DEFAULT_EXPORT_EXCLUDE_ROOT = frozenset({
    # Infrastructure
    "talaria-agent",         # repo checkout (multi-GB)
...
_RESERVED_NAMES = frozenset({
    "talaria", "default", "test", "tmp", "root", "sudo",
})

# Talaria subcommands that cannot be used as profile names/aliases
_TALARIA_SUBCOMMANDS = frozenset({
...
def _get_profiles_root() -> Path:
    ...
    Anchored to the talaria root, NOT to the current TALARIA_HOME
    (which may itself be a profile).  This ensures ``coder profile list``
    can see all profiles.

    In Docker/custom deployments where TALARIA_HOME points outside
    ``~/.talaria``, profiles live under ``TALARIA_HOME/profiles/`` so
    they persist on the mounted volume.
    """
    return _get_default_talaria_home() / "profiles"


def _get_default_talaria_home() -> Path:
    """Return the default (pre-profile) TALARIA_HOME path.

    In standard deployments this is ``~/.talaria``.
    In Docker/custom deployments where TALARIA_HOME is outside ``~/.talaria``
    (e.g. ``/opt/data``), returns TALARIA_HOME directly.
    """
    from talaria_constants import get_default_talaria_root
    return get_default_talaria_root()


def _get_active_profile_path() -> Path:
    """Return the path to the sticky active_profile file."""
    return _get_default_talaria_home() / "active_profile"
...
def validate_profile_name(name: str) -> None:
    ...
    if name == "default":
        return  # special alias for ~/.talaria
...
def get_profile_dir(name: str) -> Path:
    """Resolve a profile name to its TALARIA_HOME directory."""
    if name == "default":
        return _get_default_talaria_home()
...
def check_alias_collision(name: str) -> Optional[str]:
    ...
    Checks: reserved names, talaria subcommands, existing binaries in PATH.
    """
    if name in _RESERVED_NAMES:
        return f"'{name}' is a reserved name"
    if name in _TALARIA_SUBCOMMANDS:
        return f"'{name}' conflicts with a talaria subcommand"
...
def list_profiles() -> List[ProfileInfo]:
    ...
    # Default profile
    default_home = _get_default_talaria_home()
...
def create_profile(
    ...
    if name == "default":
        raise ValueError(
            "Cannot create a profile named 'default' — it is the built-in profile (~/.talaria)."
        )
    ...
    if clone_all and source_dir:
        # Full copy of source profile (exclude sibling ~/.talaria/profiles/)
        shutil.copytree(
            source_dir,
            profile_dir,
...
def seed_profile_skills(profile_dir: Path, quiet: bool = False) -> Optional[dict]:
    ...
    Uses subprocess because sync_skills() caches TALARIA_HOME at module level.
    ...
    try:
        result = subprocess.run(
            ...
            env={**os.environ, "TALARIA_HOME": str(profile_dir)},
...
def delete_profile(name: str, yes: bool = False) -> Path:
    ...
    if name == "default":
        raise ValueError(
            "Cannot delete the default profile (~/.talaria).\n"
            "To remove everything, use: talaria uninstall"
        )
...
def _cleanup_gateway_service(name: str, profile_dir: Path) -> None:
    ...
    # Temporarily set TALARIA_HOME so _profile_suffix resolves correctly
    ...
    try:
        ...
        if old_home is not None:
            os.environ["TALARIA_HOME"] = old_home
        elif "TALARIA_HOME" in os.environ:
            del os.environ["TALARIA_HOME"]
...
def set_active_profile(name: str) -> None:
    ...
    Writes to ``~/.talaria/active_profile``. Use ``"default"`` to clear.
    """
    validate_profile_name(name)
    if name != "default" and not profile_exists(name):
        raise FileNotFoundError(
            f"Profile '{name}' does not exist. "
            f"Create it with: talaria profile create {name}"
        )
...
def get_active_profile_name() -> str:
    """Infer the current profile name from TALARIA_HOME.

    Returns ``"default"`` if TALARIA_HOME is not set or points to ``~/.talaria``.
    Returns the profile name if TALARIA_HOME points into ``~/.talaria/profiles/<name>``.
    Returns ``"custom"`` if TALARIA_HOME is set to an unrecognized path.
    """
    from talaria_constants import get_talaria_home
    talaria_home = get_talaria_home()
    resolved = talaria_home.resolve()

    default_resolved = _get_default_talaria_home().resolve()
...
def export_profile(name: str, output_path: str) -> Path:
    ...
    if name == "default":
        # The default profile IS ~/.talaria itself — its parent is ~/ and its
        # directory name is ".talaria", not "default".  We stage a clean copy
...
def import_profile(archive_path: str, name: Optional[str] = None) -> Path:
    ...
    if not inferred_name:
        raise ValueError(
            ...
            "Specify it explicitly: talaria profile import <archive> --name <name>"
        )
    ...
    # dir.  Importing as "default" would target ~/.talaria itself — disallow
    # that and guide the user toward a named profile.
    if inferred_name == "default":
        raise ValueError(
            "Cannot import as 'default' — that is the built-in root profile (~/.talaria). "
            "Specify a different name: talaria profile import <archive> --name <name>"
        )
    ...
    with tempfile.TemporaryDirectory(prefix="talaria_profile_import_") as tmpdir:
...
def _migrate_honcho_profile_host(old_name: str, new_name: str, new_dir: Path) -> None:
    ...
    old_host = f"talaria.{old_name}"
    new_host = f"talaria.{new_name}"

    candidates = [
        new_dir / "honcho.json",
        _get_default_talaria_home() / "honcho.json",
...
def generate_bash_completion() -> str:
    """Generate a bash completion script for talaria profile names."""
    return '''# Talaria Agent profile completion
# Add to ~/.bashrc: eval "$(talaria completion bash)"

_talaria_profiles() {
    local profiles_dir="$HOME/.talaria/profiles"
...
_talaria_completion() {
    ...
    if [[ "$prev" == "-p" || "$prev" == "--profile" ]]; then
        COMPREPLY=($(compgen -W "$(_talaria_profiles)" -- "$cur"))
        return
    fi
...
complete -F _talaria_completion talaria
'''


def generate_zsh_completion() -> str:
    """Generate a zsh completion script for talaria profile names."""
    return '''#compdef talaria
# Talaria Agent profile completion
# Add to ~/.zshrc: eval "$(talaria completion zsh)"

_talaria() {
    local -a profiles
    profiles=(default)
    if [[ -d "$HOME/.talaria/profiles" ]]; then
        profiles+=("${(@f)$(ls $HOME/.talaria/profiles 2>/dev/null)}")
    fi
...
def resolve_profile_env(profile_name: str) -> str:
    """Resolve a profile name to a TALARIA_HOME path string.

    Called early in the CLI entry point, before any talaria modules
    are imported, to set the TALARIA_HOME environment variable.
    """
    ...
    if profile_name != "default" and not profile_dir.is_dir():
        raise FileNotFoundError(
            ...
            f"Create it with: talaria profile create {profile_name}"
        )
