"""On-demand supply-chain audit for Talaria Agent installs.

Scans three surfaces a Talaria user actually controls and we can map to
upstream advisories without auth or extra binaries:

1. The Talaria venv (every PyPI dist via ``importlib.metadata``).
2. Python deps declared by user-installed plugins under ``~/.talaria/plugins``
...
+ ``/v1/vulns/{id}``). Single-shot, on-demand, never daily.
...
def _discover_plugins(talaria_home: Path) -> list[Component]:
    """Python deps declared by plugins under ``~/.talaria/plugins``.
    ...
    """
    plugins_dir = talaria_home / "plugins"
...
def _discover_mcp() -> list[Component]:
    ...
    try:
        from talaria_cli.mcp_config import _get_mcp_servers
    except Exception:
        return []
...
def run_audit(
    *,
    ...
    home = talaria_home or Path(get_talaria_home())
...
def _count_components(
    *, skip_venv: bool, skip_plugins: bool, skip_mcp: bool, talaria_home: Path
) -> int:
    total = 0
    if not skip_venv:
        total += len(_discover_venv())
    if not skip_plugins:
        total += len(_discover_plugins(talaria_home))
    if not skip_mcp:
...
def cmd_security_audit(args: argparse.Namespace) -> int:
    """Implementation of `talaria security audit`."""
    home = Path(get_talaria_home())
    ...
    total = _count_components(
        skip_venv=skip_venv, skip_plugins=skip_plugins, skip_mcp=skip_mcp, talaria_home=home
    )
    if total == 0:
