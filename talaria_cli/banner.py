...
Pure display functions with no TalariaCLI state dependency.
"""
...
_UPSTREAM_REPO_URL = "https://github.com/NousResearch/talaria-agent.git"
...
def check_for_updates() -> Optional[int]:
    """Check whether a Talaria update is available.

    Two paths: if ``TALARIA_REVISION`` is set (nix builds embed it), compare
    ...
    cache_file = talaria_home / ".update_check"
    embedded_rev = os.environ.get("TALARIA_REVISION") or None
    ...
    if embedded_rev:
        ...
        repo_dir = talaria_home / "talaria-agent"
...
def _resolve_repo_dir() -> Optional[Path]:
    """Return the active Talaria git checkout, or None if this isn't a git install."""
    talaria_home = get_talaria_home()
    repo_dir = talaria_home / "talaria-agent"
    if not (repo_dir / ".git").exists():
        repo_dir = Path(__file__).parent.parent.resolve()
...
_RELEASE_URL_BASE = "https://github.com/NousResearch/talaria-agent/releases/tag"
...
def get_latest_release_tag(repo_dir: Optional[Path] = None) -> Optional[tuple]:
    ...
    Talaria checkout. Cached per-process. Release URL always points at the
    canonical NousResearch/talaria-agent repo (forks don't get a link).
    """
