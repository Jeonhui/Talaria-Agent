"""Slash command definitions and autocomplete for the Talaria CLI.
...
def _iter_plugin_command_entries() -> list[tuple[str, str, str]]:
    ...
    :func:`talaria_cli.plugins.PluginContext.register_command`. They behave
    like ``CommandDef`` entries for gateway surfacing: they appear in the
    Telegram command menu, in Slack's ``/talaria`` subcommand mapping, and
    ...
    """
    try:
        from talaria_cli.plugins import get_plugin_commands
    except Exception:
        return []
    try:
...
def telegram_menu_commands(max_commands: int = 100) -> tuple[list[tuple[str, str]], int]:
    ...
    Skills disabled for the ``"telegram"`` platform (via ``talaria skills
...
def slack_native_slashes() -> list[tuple[str, str, str]]:
    ...
    first-class slash and not a ``/talaria <verb>`` subcommand.
    ...
    avoidance. ``/talaria`` is always reserved as the first entry so the
    legacy ``/talaria <subcommand>`` form keeps working for anything that
    ...
    # Reserve /talaria as the catch-all top-level command.
    entries.append(("talaria", "Talk to Talaria or run a subcommand", "[subcommand] [args]"))
    seen.add("talaria")
...
def slack_app_manifest(request_url: str = "https://talaria-agent.local/slack/commands") -> dict[str, Any]:
...
def slack_subcommand_map() -> dict[str, str]:
    """Return subcommand -> /command mapping for Slack /talaria handler.

    Maps both canonical names and aliases so /talaria bg do stuff works
    the same as /talaria background do stuff.

    Plugin-registered slash commands are included so ``/talaria <plugin-cmd>``
