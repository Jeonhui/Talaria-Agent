"""``talaria logs`` — view and filter Talaria log files.
...
Usage examples::
    ...
    talaria logs                    # last 50 lines of agent.log
    talaria logs -f                 # follow agent.log in real time
    talaria logs errors             # last 50 lines of errors.log
    talaria logs gateway -n 100    # last 100 lines of gateway.log
    talaria logs --level WARNING    # only WARNING+ lines
    talaria logs --session abc123   # filter by session ID substring
    talaria logs --component tools  # only tool-related lines
    talaria logs --since 1h         # lines from the last hour
    talaria logs --since 30m -f     # follow, starting 30 min ago
"""
...
def tail_log(
    ...
    *,
    ...
    log_path = get_talaria_home() / "logs" / filename
    if not log_path.exists():
        print(f"Log file not found: {log_path}")
        print(f"(Logs are created when Talaria runs — try 'talaria chat' first)")
        sys.exit(1)
    ...
    if component:
        from talaria_logging import COMPONENT_PREFIXES
    ...
    if follow:
        print(f"--- {display_talaria_home()}/logs/{filename}{filter_desc} (Ctrl+C to stop) ---")
    else:
        print(f"--- {display_talaria_home()}/logs/{filename}{filter_desc} (last {num_lines}) ---")
...
def list_logs() -> None:
    ...
    if not log_dir.exists():
        print(f"No logs directory at {display_talaria_home()}/logs/")
        return

    print(f"Log files in {display_talaria_home()}/logs/:\n")
    found = False
    ...
    if not found:
        print("  (no log files yet — run 'talaria chat' to generate logs)")
