"""
Single source of truth for provider identity in Talaria Agent.
...
2. **Talaria overlays** — transport type, auth patterns, aggregator flags,
...
# -- Talaria overlay ----------------------------------------------------------
# Talaria-specific metadata that models.dev doesn't provide.

@dataclass(frozen=True)
class TalariaOverlay:
    """Talaria-specific provider metadata layered on top of models.dev."""
...
TALARIA_OVERLAYS: Dict[str, TalariaOverlay] = {
    "openai": TalariaOverlay(
        transport="openai_chat",
        base_url_override="https://api.openai.com/v1",
        base_url_env_var="OPENAI_BASE_URL",
    ),
    "openai-codex": TalariaOverlay(
        transport="codex_responses",
        auth_type="oauth_external",
        base_url_override="https://chatgpt.com/backend-api/codex",
    ),
    "lmstudio": TalariaOverlay(
        transport="openai_chat",
        auth_type="api_key",
    ...
    "anthropic": TalariaOverlay(
        transport="anthropic_messages",
        extra_env_vars=("ANTHROPIC_TOKEN", "CLAUDE_CODE_OAUTH_TOKEN"),
    ),
    "xiaomi": TalariaOverlay(
        transport="openai_chat",
...
class ProviderDef:
    ...
    doc: str = ""
    source: str = ""                      # "models.dev", "talaria", "user-config"
...
def get_provider(name: str) -> Optional[ProviderDef]:
    ...
    Resolution order:
      1. Talaria overlays (for providers not in models.dev: nous, openai-codex, etc.)
      2. models.dev catalog + Talaria overlay
    ...
    overlay = TALARIA_OVERLAYS.get(canonical)
    ...
    if mdev_info is not None:
        ...
        # Combine env vars: models.dev env + talaria extra
    ...
    if overlay is not None:
        # Talaria-only provider (not in models.dev)
        return ProviderDef(
            id=canonical,
...
def determine_api_mode(provider: str, base_url: str = "") -> str:
    ...
    # Direct provider checks for providers not in TALARIA_OVERLAYS
    if provider == "bedrock":
