"""
Dump command for talaria CLI.

Outputs a compact, plain-text summary of the user's Talaria setup
...
def _redact(value: str) -> str:
    ...
    ``talaria dump`` formats empty values as blank, not as ``"(not set)"``).
    """
...
def _gateway_status() -> str:
    ...
    try:
        from talaria_cli.gateway import get_gateway_runtime_snapshot
...
def _count_skills(talaria_home: Path) -> int:
...
def _cron_summary(talaria_home: Path) -> str:
    """Return cron jobs summary."""
    jobs_file = talaria_home / "cron" / "jobs.json"
...
def run_dump(args):
    ...
    lines.append(f"talaria_home:      {display_talaria_home()}")
    ...
    toolsets = config.get("toolsets", ["talaria-cli"])
    ...
    lines.append(f"  cron_jobs:          {_cron_summary(talaria_home)}")
    lines.append(f"  skills:             {_count_skills(talaria_home)}")
