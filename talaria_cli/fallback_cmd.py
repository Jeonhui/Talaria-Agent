"""
talaria fallback — manage the fallback provider chain.
...
Subcommands:
  talaria fallback [list]   Show the current fallback chain (default when no subcommand)
  talaria fallback add      Pick provider + model via the same picker as `talaria model`,
                           then append the selection to the chain
  talaria fallback remove   Pick an entry to delete from the chain
  talaria fallback clear    Remove all fallback entries

Storage: ``fallback_providers`` in ``~/.talaria/config.yaml`` (top-level, list of
...
def _restore_auth_active_provider(value: Any) -> None:
    ...
    try:
        from talaria_cli.auth import _auth_store_lock, _load_auth_store, _save_auth_store
        ...
        # `talaria model` to fix it.  Don't fail the fallback add.
        pass
...
def cmd_fallback_list(args) -> None:  # noqa: ARG001
    ...
    if not chain:
        ...
        print()
        print("  Add one with:  talaria fallback add")
        print()
        return
    ...
    print("  Docs: https://talaria-agent.nousresearch.com/docs/user-guide/features/fallback-providers")
    print()
...
def cmd_fallback_add(args) -> None:
    ...
    print("  `talaria model` — select the provider + model you want as a fallback.")
    print()
    ...
    print()
    print("  Run `talaria fallback list` to view, or `talaria fallback remove` to delete.")
...
def cmd_fallback(args) -> None:
    """Top-level dispatcher for ``talaria fallback [subcommand]``."""
