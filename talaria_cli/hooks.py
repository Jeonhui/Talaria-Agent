"""talaria hooks — inspect and manage shell-script hooks.

Usage::

    talaria hooks list
    talaria hooks test <event> [--for-tool X] [--payload-file F]
    talaria hooks revoke <command>
    talaria hooks doctor

Consent records live under ``~/.talaria/shell-hooks-allowlist.json`` and
hook definitions come from the ``hooks:`` block in ``~/.talaria/config.yaml``
...
def hooks_command(args) -> None:
    """Entry point for ``talaria hooks`` — dispatches to the requested action."""
    sub = getattr(args, "hooks_action", None)

    if not sub:
        print("Usage: talaria hooks {list|test|revoke|doctor}")
        print("Run 'talaria hooks --help' for details.")
        return
...
def _cmd_list(_args) -> None:
    ...
    if not specs:
        print("No shell hooks configured in ~/.talaria/config.yaml.")
        print("See `talaria hooks --help` or")
                ...
                if entry and entry.get("approved_at"):
                    ...
                    if mtime_now and mtime_at and mtime_now > mtime_at:
                        print(
                            ...
                            f"run `talaria hooks doctor` to re-validate"
                        )
        print()
...
# stdin a script sees under `talaria hooks test` and `talaria hooks doctor`
...
def _print_run_result(result: Dict[str, Any]) -> None:
    ...
    if parsed:
        print(f"      parsed (Talaria wire shape): {json.dumps(parsed)}")
    else:
...
def _doctor_one(spec, shell_hooks) -> int:
    ...
    if entry and entry.get("script_mtime_at_approval"):
        ...
        if mtime_now and mtime_at and mtime_now > mtime_at:
            ...
            print(f"      ⚠ script modified since approval "
                  ...
                  f"then `talaria hooks revoke` + re-approve to refresh")
    ...
    # is already allowlisted.  Otherwise `talaria hooks doctor` would execute
    ...
    if not entry:
        print("      ℹ skipped JSON smoke test — not allowlisted yet. "
              ...
              "then re-run `talaria hooks doctor`.")
