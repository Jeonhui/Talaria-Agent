...
def openclaw_residue_hint_cli() -> str:
    """Banner shown the first time Talaria starts and finds ``~/.openclaw/``.

    Points users at ``talaria claw migrate`` (non-destructive port of config,
    memory, and skills) first. ``talaria claw cleanup`` is mentioned as the
    ...
    return (
        ...
        "To port your config, memory, and skills over to Talaria, run "
        "`talaria claw migrate`.\n"
        "If you've already migrated and want to archive the old directory, "
        "run `talaria claw cleanup` (renames it to ~/.openclaw.pre-migration — "
