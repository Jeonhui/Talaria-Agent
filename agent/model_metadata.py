...
def _resolve_requests_verify() -> bool | str:
    ...
    by default. Talaria also honours TALARIA_CA_BUNDLE (its own convention)
    ...
    """
    for env_var in ("TALARIA_CA_BUNDLE", "REQUESTS_CA_BUNDLE", "SSL_CERT_FILE"):
...
# Minimum context length required to run Talaria Agent.  Models with fewer
...
DEFAULT_CONTEXT_LENGTHS = {
    ...
    # so these hardcoded fallbacks prevent Talaria from probing-down to
...
def _get_context_cache_path() -> Path:
    ...
    return get_talaria_home() / "context_length_cache.yaml"
...
def parse_context_limit_from_error(error_msg: str) -> Optional[int]:
    ...
    patterns = [
        ...
        # Allow underscore separators and an "exceeded" token so the common
        # "context_length_exceeded: 131072" provider error is captured.
        r'context[_\s]*(?:length|size|window)[_\s]*(?:exceeded)?\s*(?:is|of|:)?\s*(\d{4,})',
        ...
        with httpx.Client(timeout=3.0, headers=headers) as client:
            ...
            if server_type == "ollama":
                ...
                if resp.status_code == 200:
                    ...
                    # Talaria grow conversations past the runtime limit and Ollama
...
def get_model_context_length(
    ...
    if effective_provider in ("copilot", "copilot-acp", "github-copilot"):
        try:
            from talaria_cli.models import get_copilot_model_context
...
def estimate_request_tokens_rough(
    ...
    *,
    ...
    Includes the major payload buckets Talaria sends to providers:
