...
_TALARIA_MD_NAMES = (".talaria.md", "TALARIA.md")


def _find_talaria_md(cwd: Path) -> Optional[Path]:
    """Discover the nearest ``.talaria.md`` or ``TALARIA.md``.
...
DEFAULT_AGENT_IDENTITY = (
    "You are Talaria Agent, an intelligent AI assistant created by Nous Research. "
...
def _skills_prompt_snapshot_path() -> Path:
    return get_talaria_home() / ".skills_prompt_snapshot.json"
...
def build_skills_system_prompt(
    ...
    scanned alongside the local ``~/.talaria/skills/`` directory.  External dirs
    ...
    _platform_hint = (
        os.environ.get("TALARIA_PLATFORM")
        or get_session_env("TALARIA_SESSION_PLATFORM")
        or ""
    )
...
def build_nous_subscription_prompt(valid_tool_names: "set[str] | None" = None) -> str:
    ...
    lines.extend(
        [
            ...
            "Useful commands: talaria setup, talaria setup tools, talaria setup terminal, talaria status.",
        ]
    )
    return "\n".join(lines)
...
def load_soul_md() -> Optional[str]:
    """Load SOUL.md from TALARIA_HOME and return its content, or None.
    ...
    """
    try:
        from talaria_cli.config import ensure_talaria_home
        ensure_talaria_home()
    except Exception as e:
        logger.debug("Could not ensure TALARIA_HOME before loading SOUL.md: %s", e)

    soul_path = get_talaria_home() / "SOUL.md"
...
def _load_talaria_md(cwd_path: Path) -> str:
    """.talaria.md / TALARIA.md — walk to git root."""
    talaria_md_path = _find_talaria_md(cwd_path)
    if not talaria_md_path:
        return ""
    try:
        content = talaria_md_path.read_text(encoding="utf-8").strip()
        if not content:
            return ""
        content = _strip_yaml_frontmatter(content)
        rel = talaria_md_path.name
        try:
            rel = str(talaria_md_path.relative_to(cwd_path))
        except ValueError:
            pass
        content = _scan_context_content(content, rel)
        result = f"## {rel}\n\n{content}"
        return _truncate_content(result, ".talaria.md")
    except Exception as e:
        logger.debug("Could not read %s: %s", talaria_md_path, e)
        return ""
...
def build_context_files_prompt(cwd: Optional[str] = None, skip_soul: bool = False) -> str:
    ...
    Priority (first found wins — only ONE project context type is loaded):
      1. .talaria.md / TALARIA.md  (walk to git root)
      2. AGENTS.md / agents.md   (cwd only)
      3. CLAUDE.md / claude.md   (cwd only)
      4. .cursorrules / .cursor/rules/*.mdc  (cwd only)

    SOUL.md from TALARIA_HOME is independent and always included when present.
    ...
    # SOUL.md from TALARIA_HOME only — skip when already loaded as identity
    if not skip_soul:
