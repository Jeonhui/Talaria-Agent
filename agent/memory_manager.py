...
class MemoryManager:
    ...
    def initialize_all(self, session_id: str, **kwargs) -> None:
        ...
        Automatically injects ``talaria_home`` into *kwargs* so that every
        provider can resolve profile-scoped storage paths without importing
        ``get_talaria_home()`` themselves.
        """
        if "talaria_home" not in kwargs:
            from talaria_constants import get_talaria_home
            kwargs["talaria_home"] = str(get_talaria_home())
        for provider in self._providers:
            try:
