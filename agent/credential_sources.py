"""Unified removal contract for every credential source Talaria reads from.

Talaria seeds its credential pool from many places:

    env:<VAR>     — os.environ / ~/.talaria/.env
    claude_code   — ~/.claude/.credentials.json
    talaria_pkce   — ~/.talaria/.anthropic_oauth.json
    ...
    manual        — user ran `talaria auth add`
...
unify here is **removal**:
    ...
    ``talaria auth remove <provider> <N>`` must make the pool entry stay gone.

Before this module, every source had an ad-hoc removal branch in
``auth_remove_command``, and several sources had no branch at all — so
``auth remove`` silently reverted on the next ``load_pool()`` call for
qwen-cli, nous device_code (partial), talaria_pkce, copilot gh_cli, and
...
def _remove_env_source(provider: str, removed) -> RemovalResult:
    ...
    Handles three user situations:
      1. Var lives only in ~/.talaria/.env  → clear it
    ...
    """
    from talaria_cli.config import get_env_path, remove_env_value
    ...
    if shell_exported:
        result.hints.extend([
            ...
            f"(not in ~/.talaria/.env).",
            "  Unset it there (shell profile, systemd EnvironmentFile, "
            "launchd plist, etc.) or it will keep being visible to Talaria.",
            f"  The pool entry is now suppressed — Talaria will ignore "
            f"{env_var} until you run `talaria auth add {provider}`.",
        ])
    else:
        result.hints.append(
...
def _remove_claude_code(provider: str, removed) -> RemovalResult:
    ...
    work.  We just suppress it so Talaria stops reading it.
    """
    return RemovalResult(hints=[
        "Suppressed claude_code credential — it will not be re-seeded.",
        "Note: Claude Code credentials still live in ~/.claude/.credentials.json",
        "Run `talaria auth add anthropic` to re-enable if needed.",
    ])


def _remove_talaria_pkce(provider: str, removed) -> RemovalResult:
    """~/.talaria/.anthropic_oauth.json is ours — delete it outright."""
    from talaria_constants import get_talaria_home

    result = RemovalResult()
    oauth_file = get_talaria_home() / ".anthropic_oauth.json"
    if oauth_file.exists():
        try:
            oauth_file.unlink()
            result.cleaned.append("Cleared Talaria Anthropic OAuth credentials")
...
def _remove_codex_device_code(provider: str, removed) -> RemovalResult:
    ...
    the Talaria auth store is not enough — _seed_from_singletons() would
    ...
    via ``talaria auth add openai-codex``), but in both cases the re-seed
    ...
    """
    from talaria_cli.auth import suppress_credential_source
    ...
    result.hints.extend([
        ...
        "Run `talaria auth add openai-codex` to re-enable if needed.",
    ])
    return result
...
def _remove_qwen_cli(provider: str, removed) -> RemovalResult:
    ...
    return RemovalResult(hints=[
        ...
        "Run `talaria auth add qwen-oauth` to re-enable if needed.",
    ])
...
def _remove_copilot_gh(provider: str, removed) -> RemovalResult:
    ...
    Talaria stops picking the token up.
    """
    ...
    return RemovalResult(hints=[
        ...
        "Run `talaria auth add copilot` to re-enable if needed.",
    ])
