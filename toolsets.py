...
TOOLSETS = {
    ...
    # ==========================================================================
    # Full Talaria toolsets (CLI + messaging platforms)
    #
    ...
    "talaria-gateway": {
        ...
        "tools": [],
        "includes": ["talaria-telegram", "talaria-discord", "talaria-slack", "talaria-webhook"]
    }
}
...
def resolve_toolset(name: str, visited: Set[str] = None) -> List[str]:
    ...
    if not toolset:
        # Auto-generate a toolset for plugin platforms (talaria-<name>).
        # Gives them _TALARIA_CORE_TOOLS plus any tools the plugin registered
        # into a toolset matching the platform name.
        if name.startswith("talaria-"):
            platform_name = name[len("talaria-"):]
            try:
                from gateway.platform_registry import platform_registry
                if platform_registry.is_registered(platform_name):
                    plugin_tools = set(_TALARIA_CORE_TOOLS)
                    try:
                        from tools.registry import registry
                        plugin_tools.update(
