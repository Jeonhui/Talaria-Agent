#!/usr/bin/env python3
"""
Talaria Agent CLI - Interactive Terminal Interface

A beautiful command-line interface for the Talaria Agent, inspired by Claude Code.
Features ASCII art branding, interactive REPL, toolset selection, and rich formatting.

Usage:
    python cli.py                          # Start interactive mode with all tools
    python cli.py --toolsets web,terminal  # Start with specific toolsets
    python cli.py --skills talaria-agent-dev,github-auth
...
os.environ["TALARIA_QUIET"] = "1"  # Our own modules
...
from talaria_cli.banner import _format_context_length, format_banner_version_label
...
from talaria_cli.browser_connect import (
...
def load_cli_config() -> Dict[str, Any]:
    ...
    Config lookup order:
    1. ~/.talaria/config.yaml (user config - preferred)
    2. ./cli-config.yaml (project config - fallback)
    
    Environment variables take precedence over config file values.
    Returns default values if no config file exists.

    If TALARIA_IGNORE_USER_CONFIG=1 is set (via ``talaria chat --ignore-user-config``),
    the user config at ``~/.talaria/config.yaml`` is skipped entirely and only the
    built-in defaults plus the project-level ``cli-config.yaml`` (if any) are used.
    Credentials in ``.env`` are still loaded — this flag only suppresses
    behavioral/config settings.
    """
    # Check user config first ({TALARIA_HOME}/config.yaml)
    user_config_path = _talaria_home / 'config.yaml'
    project_config_path = Path(__file__).parent / 'cli-config.yaml'

    # --ignore-user-config: force-skip the user config.yaml (still honor project
    # config as a fallback so defaults stay sensible).
    ignore_user_config = os.environ.get("TALARIA_IGNORE_USER_CONFIG") == "1"
    ...
    defaults = {
        ...
        "agent": {
            ...
            "personalities": {
                ...
                "pirate": "Arrr! Ye be talkin' to Captain Talaria, the most tech-savvy pirate to sail the digital seas! Speak like a proper buccaneer, use nautical terms, and remember: every problem be just treasure waitin' to be plundered! Yo ho ho!",
                "shakespeare": "Hark! Thou speakest with an assistant most versed in the bardic arts. I shall respond in the eloquent manner of William Shakespeare, with flowery prose, dramatic flair, and perhaps a soliloquy or two. What light through yonder terminal breaks?",
                "surfer": "Duuude! You're chatting with the chillest AI on the web, bro! Everything's gonna be totally rad. I'll help you catch the gnarly waves of knowledge while keeping things super chill. Cowabunga!",
                "noir": "The rain hammered against the terminal like regrets on a guilty conscience. They call me Talaria - I solve problems, find answers, dig up the truth that hides in the shadows of your codebase. In this city of silicon and secrets, everyone's got something to hide. What's your story, pal?",
        ...
        try:
            ...
            if "model" in file_config:
                if isinstance(file_config["model"], str):
                    ...
                    # hardcoded default and TalariaCLI.__init__ checks "default" first.
            ...
            # location is model.provider (written by `talaria model`).
    ...
    if isinstance(security_config, dict):
        ...
        if redact is not None:
            os.environ["TALARIA_REDACT_SECRETS"] = str(redact).lower()
...
# Initialize centralized logging early — agent.log + errors.log in ~/.talaria/logs/.
...
from talaria_cli.banner import build_welcome_banner
from talaria_cli.commands import SlashCommandCompleter, SlashCommandAutoSuggest
...
from talaria_cli.callbacks import prompt_for_secret
...
def _setup_worktree(repo_root: str = None) -> Optional[Dict[str, str]]:
    ...
    if not repo_root:
        ...
        print("  cd into your project repo first, then run talaria -w")
        return None

    short_id = uuid.uuid4().hex[:8]
    wt_name = f"talaria-{short_id}"
    branch_name = f"talaria/{wt_name}"
...
def _run_state_db_auto_maintenance(session_db) -> None:
    ...
    :func:`talaria_cli.config.load_config` (the authoritative loader that
    ...
    try:
        ...
        from talaria_constants import get_talaria_home as _get_talaria_home
        ...
        session_db.maybe_auto_prune_and_vacuum(
            ...
            sessions_dir=_get_talaria_home() / "sessions",
        )
...
def _run_checkpoint_auto_maintenance() -> None:
    ...
    :func:`talaria_cli.config.load_config`. Honours ``auto_prune`` /
...
def _prune_stale_worktrees(repo_root: str, max_age_hours: int = 24) -> None:
    ...
    Also prunes orphaned ``talaria/*`` and ``pr-*`` local branches that
...
def _prune_orphaned_branches(repo_root: str) -> None:
    """Delete local ``talaria/talaria-*`` and ``pr-*`` branches with no worktree.

    These are auto-generated by ``talaria -w`` sessions and PR review
...
# ASCII Art - Talaria Caduceus (compact, fits in left panel)
TALARIA_CADUCEUS = """[#CD7F32]⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⢀⣀⡀⠀⣀⣀⠀⢀⣀⡀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀[/]
[#CD7F32]⠀⠀⠀⠀⠀⠀⢀⣠⣴⣾⣿⣿⣇⠸⣿⣿⠇⣸⣿⣿⣷⣦⣄⡀⠀⠀⠀⠀⠀⠀[/]
[#FFBF00]⠀⢀⣠⣴⣶⠿⠋⣩⡿⣿⡿⠻⣿⡇⢠⡄⢸⣿⠟⢿⣿⢿⣍⠙⠿⣶⣦⣄⡀⠀[/]
[#FFBF00]⠀⠀⠉⠉⠁⠶⠟⠋⠀⠉⠀⢀⣈⣁⡈⢁⣈⣁⡀⠀⠉⠀⠙⠻⠶⠈⠉⠉⠀⠀[/]
[#FFD700]⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⣴⣿⡿⠛⢁⡈⠛⢿⣿⣦⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀[/]
...
def _get_plugin_cmd_handler_names() -> set:
    ...
    try:
        from talaria_cli.plugins import get_plugin_manager
...
def save_config_value(key_path: str, value: any) -> bool:
    ...
    1. ~/.talaria/config.yaml (user config - preferred, used if it exists)
    2. ./cli-config.yaml (project config - fallback)
...
class TalariaCLI:
    """
    Interactive CLI for the Talaria Agent.
    ...
    def __init__(
        ...
        if self.model == _DEFAULT_CONFIG_MODEL:
            ...
            if "localhost" in _base_url or "127.0.0.1" in _base_url:
                from talaria_cli.runtime_provider import _auto_detect_local_model
        ...
        elif os.getenv("TALARIA_MAX_ITERATIONS"):
        ...
        # by `talaria chat --ignore-rules` in talaria_cli/main.py. When true we
        ...
        # it's shared across all Talaria processes for this TALARIA_HOME.
    ...
    def _normalize_model_for_provider(self, resolved_provider: str) -> bool:
        ...
        if resolved_provider == "copilot":
            try:
                from talaria_cli.models import copilot_model_api_mode, normalize_copilot_model_id
        ...
        if resolved_provider in {"opencode-zen", "opencode-go"}:
            try:
                from talaria_cli.models import normalize_opencode_model_id, opencode_model_api_mode
    ...
    def _ensure_runtime_credentials(self) -> bool:
        ...
        if not isinstance(api_key, str) or not api_key:
            ...
            if _has_custom_base:
                ...
                print("\n⚠️  Provider resolver returned an empty API key. "
                      "Set ANTHROPIC_API_KEY / OPENAI_API_KEY or run: talaria setup")
                return False
        if not isinstance(base_url, str) or not base_url:
            print("\n⚠️  Provider resolver returned an empty base URL. "
                  "Check your provider config or run: talaria setup")
            return False
        ...
        # `talaria chat --model <provider-name>` sends the provider name
        ...
        # If model is still empty (e.g. user ran `talaria auth add openai-codex`
        # without `talaria model`), fall back to the provider's first catalog
    ...
    def _init_agent(self, *, model_override: str = None, runtime_override: dict = None, request_overrides: dict | None = None) -> bool:
        ...
        if self._resumed and self._session_db and not self.conversation_history:
            ...
            if not session_meta:
                ...
                _cprint(f"{_DIM}Use a session ID from a previous CLI run (talaria sessions list).{_RST}")
                return False
    ...
    def show_banner(self):
        ...
        # Warn if the configured model is a Nous Talaria LLM (not agentic)
        from talaria_cli.model_switch import is_nous_talaria_non_agentic

        model_name = getattr(self, "model", "") or ""
        if is_nous_talaria_non_agentic(model_name):
            self._console_print()
            self._console_print(
                "[bold yellow]⚠  Nous Research Talaria 3 & 4 models are NOT agentic and are not "
                "designed for use with Talaria Agent.[/]"
            )
            self._console_print(
    ...
    def _try_attach_clipboard_image(self) -> bool:
        ...
        Saves the image to ~/.talaria/images/ and appends the path to
        ``_attached_images``.  Returns True if an image was attached.
        """
        from talaria_cli.clipboard import save_clipboard_image

        img_dir = get_talaria_home() / "images"
    ...
    def _handle_rollback_command(self, command: str):
        ...
        if not mgr.enabled:
            ...
            print("  Enable with: talaria --checkpoints")
    ...
    def _handle_snapshot_command(self, command: str):
        """Handle /snapshot — lightweight state snapshots for Talaria config/state.
        ...
        """
        from talaria_cli.backup import (
    ...
    def _handle_paste_command(self):
        ...
        from talaria_cli.clipboard import has_clipboard_image
    ...
    def show_help(self):
        ...
        try:
            from talaria_cli.skin_engine import get_active_help_header
    ...
    def _handle_tools_command(self, cmd: str):
        ...
        def _run_capture(ns: Namespace) -> None:
            ...
            # Buffer reports isatty()=True so color() in talaria_cli/colors.py
    ...
    def _show_recent_sessions(self, *, reason: str = "history", limit: int = 10) -> bool:
        ...
        from talaria_cli.main import _relative_time
            ...
            if self._session_db:
                try:
                    self._session_db.create_session(
                        session_id=self.session_id,
                        source=os.environ.get("TALARIA_SESSION_SOURCE", "cli"),
                        model=self.model,
                        model_config={
                            "max_iterations": self.max_turns,
                            "reasoning_config": self.reasoning_config,
    ...
    def _handle_resume_command(self, cmd_original: str) -> None:
        ...
        if not target:
            ...
            if self._show_recent_sessions(reason="resume"):
                return
            _cprint("  Tip:   Use /history or `talaria sessions list` to find sessions.")
            return
        ...
        # Resolve title or ID
        from talaria_cli.main import _resolve_session_by_name_or_id
        ...
        if not session_meta:
            ...
            _cprint("  Use /history or `talaria sessions list` to see available sessions.")
            return
    ...
    def save_conversation(self):
        """Save the current conversation to a JSON snapshot under ~/.talaria/sessions/saved/.
        ...
        DB, so the live session remains resumable via ``talaria --resume <id>``
        ...
        saved_dir = get_talaria_home() / "sessions" / "saved"
        try:
            saved_dir.mkdir(parents=True, exist_ok=True)
        except Exception as e:
            print(f"(x_x) Failed to create save directory {saved_dir}: {e}")
            return
        path = saved_dir / f"talaria_conversation_{timestamp}.json"
    ...
    def _run_curses_picker(self, title: str, items: list[str], default_index: int = 0) -> int | None:
        ...
        import threading
        from talaria_cli.curses_ui import curses_single_select
    ...
    def _handle_model_picker_selection(self, persist_global: bool = False) -> None:
        ...
        if stage == "provider":
            ...
            # (same lists as `talaria model` and gateway pickers).
            # Only fall back to the live provider catalog when the curated
            # list is empty (e.g. user-defined endpoints with no curated list).
            model_list = provider_data.get("models", [])
            if not model_list:
                try:
                    from talaria_cli.models import provider_model_ids
        ...
        if stage == "model":
            ...
            if selected < len(model_list):
                from talaria_cli.model_switch import switch_model
    ...
    def _handle_model_switch(self, cmd_original: str):
        ...
        """
        from talaria_cli.model_switch import switch_model, parse_model_flags, list_authenticated_providers
    ...
    def _should_handle_model_command_inline(self, text: str, has_images: bool = False) -> bool:
        ...
        if not text or has_images or not _looks_like_slash_command(text):
            return False
        try:
            from talaria_cli.commands import resolve_command
    ...
    def _handle_curator_command(self, cmd: str):
        ...
        Delegates to talaria_cli.curator so the CLI and the `talaria curator`
        ...
        try:
            from talaria_cli.curator import cli_main
            cli_main(tokens)
        except SystemExit:
    ...
    def _handle_skills_command(self, cmd: str):
        """Handle /skills slash command — delegates to talaria_cli.skills_hub."""
        from talaria_cli.skills_hub import handle_skills_slash
    ...
    def process_command(self, command: str) -> bool:
        ...
        # change in talaria_cli/commands.py instead of touching every dispatch site.
        from talaria_cli.commands import resolve_command as _resolve_cmd
        ...
        if canonical in ("quit", "exit", "q"):
            ...
            from talaria_cli.config import reload_env
            count = reload_env()
            ...
            try:
                ...
                if not plugins:
                    ...
                    print(f"Drop plugin directories into {display_talaria_home()}/plugins/ to get started.")
                else:
            ...
            # Check for skill slash commands (/gif-search, /axolotl, etc.)
                ...
                from talaria_cli.commands import COMMANDS
    ...
    def _handle_skin_command(self, cmd: str):
        ...
        try:
            from talaria_cli.skin_engine import list_skins, set_active_skin, get_active_skin_name
        except ImportError:
        ...
        if len(parts) < 2 or not parts[1].strip():
            ...
            print("\n  Usage: /skin <name>")
            print(f"  Custom skins: drop a YAML file in {display_talaria_home()}/skins/\n")
            return
    ...
    def _handle_footer_command(self, cmd_original: str) -> None:
        ...
        """
        from talaria_cli.config import load_config
        from talaria_cli.colors import Colors as _Colors
    ...
    def _toggle_yolo(self):
        ...
        current = bool(os.environ.get("TALARIA_YOLO_MODE"))
        if current:
            os.environ.pop("TALARIA_YOLO_MODE", None)
            _cprint(
    ...
    def _handle_busy_command(self, cmd: str):
        """Handle /busy — control what Enter does while Talaria is working.
    ...
    def _handle_fast_command(self, cmd: str):
        ...
        try:
            from talaria_cli.models import _is_anthropic_fast_model
            agent = getattr(self, "agent", None)
            model = getattr(agent, "model", None) or getattr(self, "model", None)
    ...
    def _handle_debug_command(self):
        ...
        from talaria_cli.debug import run_debug_share
    ...
    def _show_usage(self):
        ...
        if self.verbose:
            ...
            for quiet_logger in ('tools', 'run_agent', 'trajectory_compressor', 'cron', 'talaria_cli'):
    ...
    def _check_config_mcp_changes(self) -> None:
        ...
        from talaria_cli.config import get_config_path as _get_config_path
    ...
    def _reload_skills(self) -> None:
        """Reload skills: rescan ~/.talaria/skills/ and queue a note for the
        next user turn.
    ...
    def _voice_speak_response(self, text: str):
        ...
        try:
            ...
            os.makedirs(os.path.join(tempfile.gettempdir(), "talaria_voice"), exist_ok=True)
    ...
    def _print_exit_summary(self):
        ...
        if msg_count > 0:
            ...
            print(f"  talaria --resume {self.session_id}")
            if session_title:
                print(f"  talaria -c \"{session_title}\"")
            print()
    ...
    def _get_tui_prompt_symbols(self) -> tuple[str, str]:
        ...
        """
        try:
            from talaria_cli.skin_engine import get_active_prompt_symbol
    ...
    def _build_tui_style_dict(self) -> dict[str, str]:
        ...
        try:
            from talaria_cli.skin_engine import get_prompt_toolkit_style_overrides
    ...
    def run(self):
        ...
        def handle_paste(event):
            ...
            if pasted_text:
                ...
                if line_count >= 5 and not buf.text.strip().startswith('/'):
                    _paste_counter[0] += 1
                    paste_dir = _talaria_home / "pastes"
        ...
        # Dynamic prompt: shows Talaria symbol when agent is working,
        ...
        def _get_model_picker_display():
            ...
            try:
                ...
                term_rows = shutil.get_terminal_size((100, 24)).lines
            scroll_offset, visible = TalariaCLI._compute_model_picker_viewport(
        ...
        def _signal_handler(signum, frame):
            ...
            Grace window (``TALARIA_SIGTERM_GRACE``, default 1.5 s) gives
            ...
            try:
                if getattr(self, "agent", None) and getattr(self, "_agent_running", False):
                    ...
                    try:
                        _grace = float(os.getenv("TALARIA_SIGTERM_GRACE", "1.5"))
                    except (TypeError, ValueError):
                        _grace = 1.5
                    if _grace > 0:
                        time.sleep(_grace)
        ...
        try:
            ...
            print(
                ...
                "Try reinstalling Python via pyenv or Homebrew, then re-run: talaria setup"
            )
            _run_cleanup()
...
def main(
    ...
    # mode registers its own inside TalariaCLI.run(), but `-q` runs
    ...
    # so main unwinds normally.  TALARIA_SIGTERM_GRACE overrides the 1.5 s
        ...
        if quiet:
            ...
            if cli._ensure_runtime_credentials():
                ...
                if cli._init_agent(
                    ...
                    # machine-readable (no styled "Talaria" box, no tool-gen
