"""
TalariaAgentBaseEnv -- Abstract Base Environment for Talaria-Agent + Atropos

Provides the Atropos integration plumbing that all talaria-agent environments share:
- Two-mode operation (OpenAI server for Phase 1, VLLM ManagedServer for Phase 2)
- Per-group toolset/distribution resolution
- Agent loop orchestration via TalariaAgentLoop
...
# Ensure the talaria-agent repo root is on sys.path so that imports like
...
# Load API keys from talaria-agent/.env so all environments can access them
...
from environments.agent_loop import AgentResult, TalariaAgentLoop
...
# Import talaria-agent toolset infrastructure
...
class TalariaAgentEnvConfig(BaseEnvConfig):
    """
    Configuration for talaria-agent Atropos environments.
    ...
    enabled_toolsets: Optional[List[str]] = Field(
        default=None,
        description="Explicit list of talaria toolsets to enable (e.g., ['terminal', 'file', 'web']). "
    ...
    tool_call_parser: str = Field(
        ...
        "Options: talaria, mistral, llama3_json, qwen, deepseek_v3, etc.",
    )
    ...
    default_result_size_chars: int = Field(
        ...
        "to sandbox. Results exceeding this are written to /tmp/talaria-results/ "
...
class TalariaAgentBaseEnv(BaseEnv):
    """
    Abstract base environment for talaria-agent Atropos integration.
    ...
    name: Optional[str] = "talaria-agent"
    env_config_cls = TalariaAgentEnvConfig

    def __init__(
        self,
        config: TalariaAgentEnvConfig,
        ...
        # Set terminal environment variables so talaria tools pick them up.
    ...
    async def compute_reward(
        ...
        - ctx: ToolContext -- call ANY talaria-agent tool (terminal, file, web,
