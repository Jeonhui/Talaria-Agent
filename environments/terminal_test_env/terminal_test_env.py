...
Training tasks (3):
    1. Create ~/greeting.txt with "Hello from Talaria Agent"
...
TRAIN_TASKS = [
    {
        "prompt": "Create a file at ~/greeting.txt containing exactly the text: Hello from Talaria Agent",
        "verify_path": "~/greeting.txt",
        "expected_content": "Hello from Talaria Agent",
    },
    {
...
class TerminalTestEnv(TalariaAgentBaseEnv):
    ...
    def config_init(cls) -> Tuple[TerminalTestEnvConfig, List[APIServerConfig]]:
        ...
        Claude for inference. API keys loaded from ~/talaria-agent/.env.
        """
