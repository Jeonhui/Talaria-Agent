...
The PID file lives at ``{TALARIA_HOME}/gateway.pid``.  TALARIA_HOME defaults to
``~/.talaria`` but can be overridden via the environment variable.  This means
separate TALARIA_HOME directories naturally get separate PID files — a property
...
def _get_pid_path() -> Path:
    """Return the path to the gateway PID file, respecting TALARIA_HOME."""
...
def _get_lock_dir() -> Path:
    ...
    override = os.getenv("TALARIA_GATEWAY_LOCK_DIR")
    if override:
        return Path(override)
    state_home = Path(os.getenv("XDG_STATE_HOME", Path.home() / ".local" / "state"))
    return state_home / "talaria" / _LOCKS_DIRNAME
...
def _looks_like_gateway_process(pid: int) -> bool:
    """Return True when the live PID still looks like the Talaria gateway."""
...
def acquire_scoped_lock(scope: str, identity: str, metadata: Optional[dict[str, Any]] = None) -> tuple[bool, Optional[dict[str, Any]]]:
    ...
    at once (e.g. the same Telegram bot token across different TALARIA_HOME dirs).
    """
...
# enabled in the user's systemd (e.g. ``talaria.service`` + ``talaria-
# gateway.service``).
#
