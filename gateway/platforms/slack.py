...
class SlackAdapter(BasePlatformAdapter):
    ...
    async def connect(self) -> bool:
        ...
        tokens_file = get_talaria_home() / "slack_tokens.json"
        ...
        try:
            ...
            # /model work directly without /talaria prefix). A single regex
            ...
            # manifest (see `talaria slack manifest`). In Socket Mode, Slack
            ...
            if _slash_names:
                ...
                _slash_pattern = _re.compile(r"^/talaria$")

            @self._app.command(_slash_pattern)
            async def handle_talaria_command(ack, command):
                await ack()
                await self._handle_slash_command(command)

            # Register Block Kit action handlers for approval buttons
            for _action_id in (
                "talaria_approve_once",
                "talaria_approve_session",
                "talaria_approve_always",
                "talaria_deny",
            ):
            ...
            for _action_id in (
                "talaria_confirm_once",
                "talaria_confirm_always",
                "talaria_confirm_cancel",
            ):
    ...
    def _dm_top_level_threads_as_sessions(self) -> bool:
        ...
        own Talaria session — matching the per-thread behavior channels already
                    ...
                    "elements": [
                        {
                            ...
                            "style": "primary",
                            "action_id": "talaria_approve_once",
                            "value": session_key,
                        },
                        {
                            "type": "button",
                            "text": {"type": "plain_text", "text": "Allow Session"},
                            "action_id": "talaria_approve_session",
                            "value": session_key,
                        },
                        {
                            "type": "button",
                            "text": {"type": "plain_text", "text": "Always Allow"},
                            "action_id": "talaria_approve_always",
                            "value": session_key,
                        },
                        {
                            "type": "button",
                    ...
                    "elements": [
                        {
                            ...
                            "style": "primary",
                            "action_id": "talaria_confirm_once",
                            "value": value,
                        },
                        {
                            "type": "button",
                            "text": {"type": "plain_text", "text": "Always Approve"},
                            "action_id": "talaria_confirm_always",
                            "value": value,
                        },
                        {
                            "type": "button",
                            ...
                            "style": "danger",
                            "action_id": "talaria_confirm_cancel",
                            "value": value,
                        },
                    ],
                },
    ...
    async def _handle_slash_confirm_action(self, ack, body, action) -> None:
        ...
        choice_map = {
            "talaria_confirm_once": "once",
            "talaria_confirm_always": "always",
            "talaria_confirm_cancel": "cancel",
        }
    ...
    async def _handle_approval_action(self, ack, body, action) -> None:
        ...
        choice_map = {
            "talaria_approve_once": "once",
            "talaria_approve_session": "session",
            "talaria_approve_always": "always",
            "talaria_deny": "deny",
        }
    ...
    async def _handle_slash_command(self, command: dict) -> None:
        ...
        The legacy ``/talaria <subcommand> [args]`` form is preserved for
        backward compatibility with older workspace manifests and for users
        who want a single entry point for free-form questions (``/talaria
        ...
        if slash_name in ("talaria", ""):
            # Legacy /talaria <subcommand> [args] routing + free-form questions.
            # Empty slash_name falls into this branch for backward compat
            # with any caller that didn't populate command["command"].
            from talaria_cli.commands import slack_subcommand_map
