"""Persistent slash-command worker — one TalariaCLI per TUI session.
...
from cli import TalariaCLI
from rich.console import Console


def _run(cli: TalariaCLI, command: str) -> str:
...
def main():
    ...
    os.environ["TALARIA_SESSION_KEY"] = args.session_key
    os.environ["TALARIA_INTERACTIVE"] = "1"

    with contextlib.redirect_stdout(io.StringIO()), contextlib.redirect_stderr(io.StringIO()):
        cli = TalariaCLI(model=args.model or None, compact=True, resume=args.session_key, verbose=False)
