#!/usr/bin/env python3
"""Shared CLI/runtime configuration loader.

Extracted from the (removed) interactive ``cli.py`` so the gateway, callbacks,
and delegation tooling can read ``CLI_CONFIG`` / ``save_config_value`` without
pulling in the terminal-chat REPL.

Importing this module runs :func:`load_cli_config` once and bridges the
resulting config into environment variables (TERMINAL_*, BROWSER_*,
AUXILIARY_*, TALARIA_REDACT_SECRETS) — the gateway relies on that side effect
when it lazily imports this module at runtime (see issue #10817).
"""
...
def load_cli_config() -> Dict[str, Any]:
    ...
    If TALARIA_IGNORE_USER_CONFIG=1 is set, the user config at
    ``~/.talaria/config.yaml`` is skipped entirely and only the built-in
    defaults plus the project-level ``cli-config.yaml`` (if any) are used.
    ...
    # This prevents a lazy import of this module during gateway runtime from
...
# Load configuration at module import (bridges config → env vars).
