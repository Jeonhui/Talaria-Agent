"""
Talaria MCP Server — expose messaging conversations as MCP tools.
...
logger = logging.getLogger("talaria.mcp_serve")
...
def _get_sessions_dir() -> Path:
    """Return the sessions directory using TALARIA_HOME."""
    try:
        from talaria_constants import get_talaria_home
        return get_talaria_home() / "sessions"
    except ImportError:
        return Path(os.environ.get("TALARIA_HOME", Path.home() / ".talaria")) / "sessions"
...
def _load_channel_directory() -> dict:
    ...
    try:
        from talaria_constants import get_talaria_home
        directory_file = get_talaria_home() / "channel_directory.json"
    except ImportError:
        directory_file = Path(
            os.environ.get("TALARIA_HOME", Path.home() / ".talaria")
...
class EventBridge:
    ...
    def _poll_once(self, db):
        ...
        try:
            from talaria_constants import get_talaria_home
            db_file = get_talaria_home() / "state.db"
        except ImportError:
            db_file = Path(os.environ.get("TALARIA_HOME", Path.home() / ".talaria")) / "state.db"
...
def create_mcp_server(event_bridge: Optional[EventBridge] = None) -> "FastMCP":
    """Create and return the Talaria MCP server with all tools registered."""
...
def run_mcp_server(verbose: bool = False) -> None:
    """Start the Talaria MCP server on stdio."""
    if not _MCP_SERVER_AVAILABLE:
        print(
