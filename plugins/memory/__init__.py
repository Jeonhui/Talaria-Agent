...
1. Bundled providers: ``plugins/memory/<name>/`` (shipped with talaria-agent)
2. User-installed providers: ``$TALARIA_HOME/plugins/<name>/``
...
def _get_user_plugins_dir() -> Optional[Path]:
    """Return ``$TALARIA_HOME/plugins/`` or None if unavailable."""
    try:
        from talaria_constants import get_talaria_home
        d = get_talaria_home() / "plugins"
...
def load_memory_provider(name: str) -> Optional["MemoryProvider"]:
    ...
    (``$TALARIA_HOME/plugins/<name>/``) directories.  Bundled takes
...
def _load_provider_from_dir(provider_dir: Path) -> Optional["MemoryProvider"]:
    ...
    module_name = f"plugins.memory.{name}" if _is_bundled else f"_talaria_user_memory.{name}"
...
def discover_plugin_cli_commands() -> List[dict]:
    ...
    module_name = f"plugins.memory.{active_provider}.cli" if _is_bundled else f"_talaria_user_memory.{active_provider}.cli"
    try:
