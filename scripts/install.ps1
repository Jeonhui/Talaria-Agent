# ============================================================================
# Talaria Agent Installer for Windows
# ============================================================================
# Installation script for Windows (PowerShell).
# Uses uv for fast Python provisioning and package management.
#
# Usage:
#   irm https://raw.githubusercontent.com/NousResearch/talaria-agent/main/scripts/install.ps1 | iex
#
...
param(
    ...
    [switch]$SkipSetup,
    [string]$Branch = "main",
    [string]$TalariaHome = "$env:LOCALAPPDATA\talaria",
    [string]$InstallDir = "$env:LOCALAPPDATA\talaria\talaria-agent"
)
...
$RepoUrlSsh = "git@github.com:NousResearch/talaria-agent.git"
$RepoUrlHttps = "https://github.com/NousResearch/talaria-agent.git"
$PythonVersion = "3.11"
$NodeVersion = "22"
...
function Write-Banner {
    ...
    Write-Host "│             ⚕ Talaria Agent Installer                    │" -ForegroundColor Magenta
    Write-Host "├─────────────────────────────────────────────────────────┤" -ForegroundColor Magenta
...
function Test-Node {
    ...
    $managedNode = "$TalariaHome\node\node.exe"
    if (Test-Path $managedNode) {
        $version = & $managedNode --version
        $env:Path = "$TalariaHome\node;$env:Path"
        Write-Success "Node.js $version found (Talaria-managed)"
        $script:HasNode = $true
        return $true
    }
    ...
    # Fallback: download binary zip to ~/.talaria/node/
    ...
    try {
        ...
        if ($zipName) {
            ...
            $tmpDir = "$env:TEMP\talaria-node-extract"
            ...
            if ($extractedDir) {
                if (Test-Path "$TalariaHome\node") { Remove-Item -Recurse -Force "$TalariaHome\node" }
                Move-Item $extractedDir.FullName "$TalariaHome\node"
                $env:Path = "$TalariaHome\node;$env:Path"

                $version = & "$TalariaHome\node\node.exe" --version
                Write-Success "Node.js $version installed to ~/.talaria/node/"
                $script:HasNode = $true
...
function Install-Repository {
    ...
    if (Test-Path $InstallDir) {
        ...
        if (-not $cloneSuccess) {
            ...
            try {
                $zipUrl = "https://github.com/NousResearch/talaria-agent/archive/refs/heads/$Branch.zip"
                $zipPath = "$env:TEMP\talaria-agent-$Branch.zip"
                $extractPath = "$env:TEMP\talaria-agent-extract"
...
function Set-PathVariable {
    Write-Info "Setting up talaria command..."
    
    if ($NoVenv) {
        $talariaBin = "$InstallDir"
    } else {
        $talariaBin = "$InstallDir\venv\Scripts"
    }
    
    # Add the venv Scripts dir to user PATH so talaria is globally available
    # On Windows, the talaria.exe in venv\Scripts\ has the venv Python baked in
    $currentPath = [Environment]::GetEnvironmentVariable("Path", "User")
    
    if ($currentPath -notlike "*$talariaBin*") {
        [Environment]::SetEnvironmentVariable(
            "Path",
            "$talariaBin;$currentPath",
            "User"
        )
        Write-Success "Added to user PATH: $talariaBin"
    } else {
        Write-Info "PATH already configured"
    }
    
    # Set TALARIA_HOME so the Python code finds config/data in the right place.
    # Only needed on Windows where we install to %LOCALAPPDATA%\talaria instead
    # of the Unix default ~/.talaria
    $currentTalariaHome = [Environment]::GetEnvironmentVariable("TALARIA_HOME", "User")
    if (-not $currentTalariaHome -or $currentTalariaHome -ne $TalariaHome) {
        [Environment]::SetEnvironmentVariable("TALARIA_HOME", $TalariaHome, "User")
        Write-Success "Set TALARIA_HOME=$TalariaHome"
    }
    $env:TALARIA_HOME = $TalariaHome
    
    # Update current session
    $env:Path = "$talariaBin;$env:Path"
    
    Write-Success "talaria command ready"
}

function Copy-ConfigTemplates {
    Write-Info "Setting up configuration files..."
    
    # Create ~/.talaria directory structure
    New-Item -ItemType Directory -Force -Path "$TalariaHome\cron" | Out-Null
    New-Item -ItemType Directory -Force -Path "$TalariaHome\sessions" | Out-Null
    New-Item -ItemType Directory -Force -Path "$TalariaHome\logs" | Out-Null
    New-Item -ItemType Directory -Force -Path "$TalariaHome\pairing" | Out-Null
    New-Item -ItemType Directory -Force -Path "$TalariaHome\hooks" | Out-Null
    New-Item -ItemType Directory -Force -Path "$TalariaHome\image_cache" | Out-Null
    New-Item -ItemType Directory -Force -Path "$TalariaHome\audio_cache" | Out-Null
    New-Item -ItemType Directory -Force -Path "$TalariaHome\memories" | Out-Null
    New-Item -ItemType Directory -Force -Path "$TalariaHome\skills" | Out-Null

    
    # Create .env
    $envPath = "$TalariaHome\.env"
    if (-not (Test-Path $envPath)) {
        $examplePath = "$InstallDir\.env.example"
        if (Test-Path $examplePath) {
            Copy-Item $examplePath $envPath
            Write-Success "Created ~/.talaria/.env from template"
        } else {
            New-Item -ItemType File -Force -Path $envPath | Out-Null
            Write-Success "Created ~/.talaria/.env"
        }
    } else {
        Write-Info "~/.talaria/.env already exists, keeping it"
    }
    
    # Create config.yaml
    $configPath = "$TalariaHome\config.yaml"
    if (-not (Test-Path $configPath)) {
        $examplePath = "$InstallDir\cli-config.yaml.example"
        if (Test-Path $examplePath) {
            Copy-Item $examplePath $configPath
            Write-Success "Created ~/.talaria/config.yaml from template"
        }
    } else {
        Write-Info "~/.talaria/config.yaml already exists, keeping it"
    }
    
    # Create SOUL.md if it doesn't exist (global persona file)
    $soulPath = "$TalariaHome\SOUL.md"
...
"@ | Set-Content -Path $soulPath -Encoding UTF8
        Write-Success "Created ~/.talaria/SOUL.md (edit to customize personality)"
    }
    
    Write-Success "Configuration directory ready: ~/.talaria/"
    
    # Seed bundled skills into ~/.talaria/skills/ (manifest-based, one-time per skill)
    Write-Info "Syncing bundled skills to ~/.talaria/skills/ ..."
    $pythonExe = "$InstallDir\venv\Scripts\python.exe"
    if (Test-Path $pythonExe) {
        try {
            & $pythonExe "$InstallDir\tools\skills_sync.py" 2>$null
            Write-Success "Skills synced to ~/.talaria/skills/"
        } catch {
            # Fallback: simple directory copy
            $bundledSkills = "$InstallDir\skills"
            $userSkills = "$TalariaHome\skills"
            if ((Test-Path $bundledSkills) -and -not (Get-ChildItem $userSkills -Exclude '.bundled_manifest' -ErrorAction SilentlyContinue)) {
                Copy-Item -Path "$bundledSkills\*" -Destination $userSkills -Recurse -Force -ErrorAction SilentlyContinue
                Write-Success "Skills copied to ~/.talaria/skills/"
            }
        }
    }
}
...
function Install-NodeDeps {
    ...
    if (Test-Path "$tuiDir\package.json") {
        ...
        try {
            ...
            Write-Warn "TUI npm install failed (talaria --tui may not work)"
        }
        Pop-Location
    }
...
function Invoke-SetupWizard {
    ...
    if (-not $NoVenv) {
        & ".\venv\Scripts\python.exe" -m talaria_cli.main setup
    } else {
        python -m talaria_cli.main setup
    }
...
function Start-GatewayIfConfigured {
    ...
    $talariaCmd = "$InstallDir\venv\Scripts\talaria.exe"
    if (-not (Test-Path $talariaCmd)) {
        $talariaCmd = "talaria"
    }

    # If WhatsApp is enabled but not yet paired, run foreground for QR scan
    $whatsappEnabled = $content | Where-Object { $_ -match "^WHATSAPP_ENABLED=true" }
    $whatsappSession = "$TalariaHome\whatsapp\session\creds.json"
    if ($whatsappEnabled -and -not (Test-Path $whatsappSession)) {
        Write-Host ""
        Write-Info "WhatsApp is enabled but not yet paired."
        Write-Info "Running 'talaria whatsapp' to pair via QR code..."
        Write-Host ""
    ...
    if ($response -eq "" -or $response -match "^[Yy]") {
        ...
        try {
            $logFile = "$TalariaHome\logs\gateway.log"
            Start-Process -FilePath $talariaCmd -ArgumentList "gateway" `
                -RedirectStandardOutput $logFile `
                -RedirectStandardError "$TalariaHome\logs\gateway-error.log" `
                -WindowStyle Hidden
            Write-Success "Gateway started! Your bot is now online."
            Write-Info "Logs: $logFile"
            Write-Info "To stop: close the gateway process from Task Manager"
        } catch {
            Write-Warn "Failed to start gateway. Run manually: talaria gateway"
        }
    } else {
        Write-Info "Skipped. Start the gateway later with: talaria gateway"
    }
}
...
function Write-Completion {
    ...
    Write-Host "$TalariaHome\config.yaml"
    Write-Host "   API Keys:  " -NoNewline -ForegroundColor Yellow
    Write-Host "$TalariaHome\.env"
    Write-Host "   Data:      " -NoNewline -ForegroundColor Yellow
    Write-Host "$TalariaHome\cron\, sessions\, logs\"
    Write-Host "   Code:      " -NoNewline -ForegroundColor Yellow
    Write-Host "$TalariaHome\talaria-agent\"
    Write-Host ""
    ...
    Write-Host ""
    Write-Host "   talaria              " -NoNewline -ForegroundColor Green
    Write-Host "Start chatting"
    Write-Host "   talaria setup        " -NoNewline -ForegroundColor Green
    Write-Host "Configure API keys & settings"
    Write-Host "   talaria config       " -NoNewline -ForegroundColor Green
    Write-Host "View/edit configuration"
    Write-Host "   talaria config edit  " -NoNewline -ForegroundColor Green
    Write-Host "Open config in editor"
    Write-Host "   talaria gateway      " -NoNewline -ForegroundColor Green
    Write-Host "Start messaging gateway (Telegram, Discord, etc.)"
    Write-Host "   talaria update       " -NoNewline -ForegroundColor Green
...
try {
    ...
    Write-Host "  Invoke-WebRequest -Uri 'https://raw.githubusercontent.com/NousResearch/talaria-agent/main/scripts/install.ps1' -OutFile install.ps1" -ForegroundColor Yellow
