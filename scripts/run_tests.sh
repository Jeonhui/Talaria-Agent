#!/usr/bin/env bash
# Canonical test runner for talaria-agent. Run this instead of calling
...
VENV=""
for candidate in "$REPO_ROOT/.venv" "$REPO_ROOT/venv" "$HOME/.talaria/talaria-agent/venv"; do
...
# Unset TALARIA_* behavioral vars too.
unset TALARIA_YOLO_MODE TALARIA_INTERACTIVE TALARIA_QUIET TALARIA_TOOL_PROGRESS \
      TALARIA_TOOL_PROGRESS_MODE TALARIA_MAX_ITERATIONS TALARIA_SESSION_PLATFORM \
      TALARIA_SESSION_CHAT_ID TALARIA_SESSION_CHAT_NAME TALARIA_SESSION_THREAD_ID \
      TALARIA_SESSION_SOURCE TALARIA_SESSION_KEY TALARIA_GATEWAY_SESSION \
      TALARIA_PLATFORM TALARIA_INFERENCE_PROVIDER TALARIA_MANAGED TALARIA_DEV \
      TALARIA_CONTAINER TALARIA_EPHEMERAL_SYSTEM_PROMPT TALARIA_TIMEZONE \
      TALARIA_REDACT_SECRETS TALARIA_BACKGROUND_NOTIFICATIONS TALARIA_EXEC_ASK \
      TALARIA_HOME_MODE 2>/dev/null || true
...
WORKERS="${TALARIA_TEST_WORKERS:-4}"
