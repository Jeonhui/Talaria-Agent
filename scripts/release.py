...
VERSION_FILE = REPO_ROOT / "talaria_cli" / "__init__.py"
...
AUTHOR_MAP = {
    ...
    "274096618+talaria-agent-dhabibi@users.noreply.github.com": "dhabibi",
    ...
    "talaria@nousresearch.com": "NousResearch",
...
def parse_coauthors(body: str) -> list:
    ...
    _ignored_emails = {"noreply@anthropic.com", "noreply@github.com",
                       "cursoragent@cursor.com", "talaria@nousresearch.com"}
...
def generate_changelog(commits, tag_name, semver, repo_url="https://github.com/NousResearch/talaria-agent",
    ...
    if first_release:
        ...
        lines.append("> for Talaria Agent. See below for everything included in this initial release.")
        lines.append("")
...
def main():
    parser = argparse.ArgumentParser(description="Talaria Agent Release Tool")
    ...
    print(f"{'='*60}")
    print(f"  Talaria Agent Release Preview")
    print(f"{'='*60}")
    ...
    if args.publish:
        ...
        tag_result = git_result(
            "tag", "-a", tag_name, "-m",
            f"Talaria Agent v{new_version} ({calver_date})\n\nWeekly release"
        )
        ...
        gh_cmd = [
            ...
            "--title", f"Talaria Agent v{new_version} ({calver_date})",
        ...
        if result and result.returncode == 0:
            ...
            print(
                f"    gh release create {tag_name} --title 'Talaria Agent v{new_version} ({calver_date})' "
