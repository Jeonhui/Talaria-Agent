#!/usr/bin/env python3
"""Drive the Talaria TUI under TALARIA_DEV_PERF and summarize the pipeline.
...
~30 Hz (matching xterm key-repeat), summarizes ~/.talaria/perf.log on exit.

The --tui build must exist (run `npm run build` in ui-tui first). This script
launches `node dist/entry.js` directly with TALARIA_TUI_RESUME set so it
bypasses the talaria_cli wrapper — we want repeatable timing, not the CLI's
session-picker flow.

Environment overrides:
  TALARIA_PERF_LOG     (default ~/.talaria/perf.log)
  TALARIA_PERF_NODE    (default node from $PATH)
  TALARIA_TUI_DIR      (default /home/bb/talaria-agent/ui-tui)

Exit code is 0 if the harness ran and parsed results, 2 if the TUI crashed
or produced no perf data (suggests TALARIA_DEV_PERF wiring is broken).
"""
...
DEFAULT_TUI_DIR = Path(os.environ.get("TALARIA_TUI_DIR", "/home/bb/talaria-agent/ui-tui"))
DEFAULT_LOG = Path(os.environ.get("TALARIA_PERF_LOG", str(Path.home() / ".talaria" / "perf.log")))
DEFAULT_STATE_DB = Path.home() / ".talaria" / "state.db"
...
def format_report(data: dict[str, Any]) -> str:
    ...
    if not react:
        out.append("  (no react events — TALARIA_DEV_PERF wired? threshold too high?)")
    else:
...
def run_once(args: argparse.Namespace) -> dict[str, Any]:
    ...
    env = os.environ.copy()
    env["TALARIA_DEV_PERF"] = "1"
    env["TALARIA_DEV_PERF_MS"] = str(args.threshold_ms)
    env["TALARIA_DEV_PERF_LOG"] = str(log)
    env["TALARIA_TUI_RESUME"] = sid
    ...
    node = os.environ.get("TALARIA_PERF_NODE", "node")
...
def main() -> int:
    ...
    p.add_argument("--threshold-ms", type=float, default=0.0, help="TALARIA_DEV_PERF_MS (0 = capture all)")
...
def loop_mode(args: argparse.Namespace) -> int:
    ...
    pkg_root = tui_dir / "packages" / "talaria-ink" / "src"
