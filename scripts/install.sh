...
# Configuration
REPO_URL_SSH="git@github.com:NousResearch/talaria-agent.git"
REPO_URL_HTTPS="https://github.com/NousResearch/talaria-agent.git"
TALARIA_HOME="${TALARIA_HOME:-$HOME/.talaria}"
# INSTALL_DIR is resolved AFTER arg parsing and OS detection so we can pick an
# FHS-style layout for root installs.  Track whether the user gave us an
# explicit directory — if so we never override it.
if [ -n "${TALARIA_INSTALL_DIR:-}" ]; then
    INSTALL_DIR="$TALARIA_INSTALL_DIR"
...
#   code at /usr/local/lib/talaria-agent, command at /usr/local/bin/talaria,
#   data still at /root/.talaria (TALARIA_HOME).  Matches Claude Code / Codex CLI
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        -h|--help)
            ...
            echo "                   default (non-root):  ~/.talaria/talaria-agent"
            echo "                   default (root, Linux): /usr/local/lib/talaria-agent"
            echo "  --talaria-home PATH  Data directory (default: ~/.talaria, or \$TALARIA_HOME)"
            echo "  -h, --help     Show this help"
            echo ""
            echo "Notes:"
            echo "  When running as root on Linux, Talaria installs the code under"
            echo "  /usr/local/lib/talaria-agent and links the command into"
            echo "  /usr/local/bin/talaria (FHS layout — matches Claude Code / Codex CLI)."
            echo "  Data, config, sessions, and logs still live in \$TALARIA_HOME"
            echo "  (default /root/.talaria).  This keeps Docker bind-mounted volumes"
            echo "  small and ensures the command is on PATH for all shells."
            echo "  Existing installs at \$TALARIA_HOME/talaria-agent are preserved in-place."
            exit 0
            ;;
        *)
...
# Decide where the repo checkout + venv live, and where the `talaria` command
# symlink goes.  Called after detect_os so $OS/$DISTRO are known.
#
# Defaults:
#   - Non-root, any OS:       INSTALL_DIR = $TALARIA_HOME/talaria-agent
#                             command link in $HOME/.local/bin
#   - Termux (any uid):       INSTALL_DIR = $TALARIA_HOME/talaria-agent
#                             command link in $PREFIX/bin (already on PATH)
#   - Root on Linux (new):    INSTALL_DIR = /usr/local/lib/talaria-agent
#                             command link in /usr/local/bin
#                             (unless a legacy install already exists at
#                              $TALARIA_HOME/talaria-agent — then preserve it)
#
# Always no-op when the user set --dir or $TALARIA_INSTALL_DIR.
resolve_install_layout() {
    ...
    # Termux: package manager manages /data/data/..., keep code in TALARIA_HOME.
    if is_termux; then
        INSTALL_DIR="$TALARIA_HOME/talaria-agent"
        return 0
    fi
    ...
    if [ "$OS" = "linux" ] && [ "$(id -u)" -eq 0 ]; then
        if [ -d "$TALARIA_HOME/talaria-agent/.git" ]; then
            INSTALL_DIR="$TALARIA_HOME/talaria-agent"
            log_info "Existing install detected at $INSTALL_DIR — keeping legacy layout"
            log_info "  (new root installs use /usr/local/lib/talaria-agent)"
            return 0
        fi
        INSTALL_DIR="/usr/local/lib/talaria-agent"
        ROOT_FHS_LAYOUT=true
        log_info "Root install on Linux — using FHS layout"
        log_info "  Code:    $INSTALL_DIR"
        log_info "  Command: /usr/local/bin/talaria"
        log_info "  Data:    $TALARIA_HOME (unchanged)"
        return 0
    fi
...
get_talaria_command_path() {
    local link_dir
    link_dir="$(get_command_link_dir)"
    if [ -x "$link_dir/talaria" ]; then
        echo "$link_dir/talaria"
    else
        echo "talaria"
    fi
}
...
detect_os() {
    case "$(uname -s)" in
        ...
        CYGWIN*|MINGW*|MSYS*)
            ...
            log_info "  irm https://raw.githubusercontent.com/NousResearch/talaria-agent/main/scripts/install.ps1 | iex"
            exit 1
            ;;
        *)
            OS="unknown"
...
check_node() {
    ...
    if [ -x "$TALARIA_HOME/node/bin/node" ]; then
        export PATH="$TALARIA_HOME/node/bin:$PATH"
        local found_ver=$("$TALARIA_HOME/node/bin/node" --version)
        log_success "Node.js $found_ver found (Talaria-managed)"
        HAS_NODE=true
        return 0
    fi
...
install_node() {
    ...
    log_info "Extracting to ~/.talaria/node/..."
    ...
    # Place into ~/.talaria/node/ and symlink binaries to ~/.local/bin/
    rm -rf "$TALARIA_HOME/node"
    mkdir -p "$TALARIA_HOME"
    mv "$extracted_dir" "$TALARIA_HOME/node"
    rm -rf "$tmp_dir"

    mkdir -p "$HOME/.local/bin"
    ln -sf "$TALARIA_HOME/node/bin/node" "$HOME/.local/bin/node"
    ln -sf "$TALARIA_HOME/node/bin/npm"  "$HOME/.local/bin/npm"
    ln -sf "$TALARIA_HOME/node/bin/npx"  "$HOME/.local/bin/npx"

    export PATH="$TALARIA_HOME/node/bin:$PATH"

    local installed_ver
    installed_ver=$("$TALARIA_HOME/node/bin/node" --version 2>/dev/null)
    log_success "Node.js $installed_ver installed to ~/.talaria/node/"
    HAS_NODE=true
}
    ...
    if [ -n "$pkg_install" ]; then
        ...
        # sudo needs password — ask once for everything
            ...
            if [ "$IS_INTERACTIVE" = true ]; then
                ...
                log_info "Talaria Agent itself does not require or retain root access."
    ...
    if [ -d "$INSTALL_DIR" ]; then
        if [ -d "$INSTALL_DIR/.git" ]; then
            ...
            if [ -n "$(git status --porcelain)" ]; then
                local stash_name
                stash_name="talaria-install-autostash-$(date -u +%Y%m%d-%H%M%S)"
            ...
            if [ -n "$autostash_ref" ]; then
                ...
                if [ "$restore_now" = "yes" ]; then
                    ...
                    if git stash apply "$autostash_ref"; then
                        ...
                        log_warn "Review git diff / git status if Talaria behaves unexpectedly."
                    else
...
setup_path() {
    log_info "Setting up talaria command..."

    if [ "$USE_VENV" = true ]; then
        TALARIA_BIN="$INSTALL_DIR/venv/bin/talaria"
    else
        TALARIA_BIN="$(which talaria 2>/dev/null || echo "")"
        if [ -z "$TALARIA_BIN" ]; then
            log_warn "talaria not found on PATH after install"
            return 0
        fi
    fi

    # Verify the entry point script was actually generated
    if [ ! -x "$TALARIA_BIN" ]; then
        log_warn "talaria entry point not found at $TALARIA_BIN"
    ...
    # Create a user-facing shim for the talaria command.
    mkdir -p "$command_link_dir"
    ln -sf "$TALARIA_BIN" "$command_link_dir/talaria"
    log_success "Symlinked talaria → $command_link_display_dir/talaria"

    if [ "$DISTRO" = "termux" ]; then
        export PATH="$command_link_dir:$PATH"
        log_info "$command_link_display_dir is the native Termux command path"
        log_success "talaria command ready"
        return 0
    fi
    ...
    if [ "$ROOT_FHS_LAYOUT" = true ]; then
        ...
        if env -i HOME="$HOME" TERM="${TERM:-dumb}" bash -i -c 'command -v talaria' \
                >/dev/null 2>&1; then
        ...
        log_info "talaria not on PATH in non-login shells (common on RHEL-family)"
        PATH_LINE='export PATH="/usr/local/bin:$PATH"'
        PATH_COMMENT='# Talaria Agent — ensure /usr/local/bin is on PATH (RHEL non-login shells)'
    ...
    if ! echo "$PATH" | tr ':' '\n' | grep -q "^$command_link_dir$"; then
        ...
        for SHELL_CONFIG in "${SHELL_CONFIGS[@]}"; do
            if ! grep -v '^[[:space:]]*#' "$SHELL_CONFIG" 2>/dev/null | grep -qE 'PATH=.*\.local/bin'; then
                echo "" >> "$SHELL_CONFIG"
                echo "# Talaria Agent — ensure ~/.local/bin is on PATH" >> "$SHELL_CONFIG"
                echo "$PATH_LINE" >> "$SHELL_CONFIG"
        ...
        if [ "$IS_FISH" = "true" ]; then
            if ! grep -q 'fish_add_path.*\.local/bin' "$FISH_CONFIG" 2>/dev/null; then
                echo "" >> "$FISH_CONFIG"
                echo "# Talaria Agent — ensure ~/.local/bin is on PATH" >> "$FISH_CONFIG"
    ...
    # Export for current session so talaria works immediately
    export PATH="$command_link_dir:$PATH"
...
copy_config_templates() {
    ...
    # Create ~/.talaria directory structure (config at top level, code in subdir)
    mkdir -p "$TALARIA_HOME"/{cron,sessions,logs,pairing,hooks,image_cache,audio_cache,memories,skills}

    # Create .env at ~/.talaria/.env (top level, easy to find)
    if [ ! -f "$TALARIA_HOME/.env" ]; then
        if [ -f "$INSTALL_DIR/.env.example" ]; then
            cp "$INSTALL_DIR/.env.example" "$TALARIA_HOME/.env"
            log_success "Created ~/.talaria/.env from template"
        else
            touch "$TALARIA_HOME/.env"
            log_success "Created ~/.talaria/.env"
        fi
    else
        log_info "~/.talaria/.env already exists, keeping it"
    fi

    # Create config.yaml at ~/.talaria/config.yaml (top level, easy to find)
    if [ ! -f "$TALARIA_HOME/config.yaml" ]; then
        if [ -f "$INSTALL_DIR/cli-config.yaml.example" ]; then
            cp "$INSTALL_DIR/cli-config.yaml.example" "$TALARIA_HOME/config.yaml"
            log_success "Created ~/.talaria/config.yaml from template"
        fi
    else
        log_info "~/.talaria/config.yaml already exists, keeping it"
    fi

    # Create SOUL.md if it doesn't exist (global persona file)
    if [ ! -f "$TALARIA_HOME/SOUL.md" ]; then
        cat > "$TALARIA_HOME/SOUL.md" << 'SOUL_EOF'
# Talaria Agent Persona

<!--
This file defines the agent's personality and tone.
The agent will embody whatever you write here.
Edit this to customize how Talaria communicates with you.
...
-->
SOUL_EOF
        log_success "Created ~/.talaria/SOUL.md (edit to customize personality)"
    fi

    log_success "Configuration directory ready: ~/.talaria/"

    # Seed bundled skills into ~/.talaria/skills/ (manifest-based, one-time per skill)
    log_info "Syncing bundled skills to ~/.talaria/skills/ ..."
    if "$INSTALL_DIR/venv/bin/python" "$INSTALL_DIR/tools/skills_sync.py" 2>/dev/null; then
        log_success "Skills synced to ~/.talaria/skills/"
    else
        # Fallback: simple directory copy if Python sync fails
        if [ -d "$INSTALL_DIR/skills" ] && [ ! "$(ls -A "$TALARIA_HOME/skills/" 2>/dev/null | grep -v '.bundled_manifest')" ]; then
            cp -r "$INSTALL_DIR/skills/"* "$TALARIA_HOME/skills/" 2>/dev/null || true
            log_success "Skills copied to ~/.talaria/skills/"
        fi
    fi
}
...
install_node_deps() {
    ...
    if [ -f "$INSTALL_DIR/package.json" ]; then
        ...
        case "$DISTRO" in
            ubuntu|debian|raspbian|pop|linuxmint|elementary|zorin|kali|parrot)
                ...
                log_info "This is standard Playwright setup — Talaria itself does not require root access."
    ...
    if [ -f "$INSTALL_DIR/ui-tui/package.json" ]; then
        ...
        npm install --silent 2>/dev/null || {
            log_warn "TUI npm install failed (talaria --tui may not work)"
        }
...
run_setup_wizard() {
    ...
    if ! (: </dev/tty) 2>/dev/null; then
        log_info "Setup wizard skipped (no terminal available). Run 'talaria setup' after install."
        return 0
    fi
    ...
    # Run talaria setup using the venv Python directly (no activation needed).
    # Redirect stdin from /dev/tty so interactive prompts work when piped from curl.
    if [ "$USE_VENV" = true ]; then
        "$INSTALL_DIR/venv/bin/python" -m talaria_cli.main setup < /dev/tty
    else
        python -m talaria_cli.main setup < /dev/tty
    fi
}

maybe_start_gateway() {
    # Check if any messaging platform tokens were configured
    ENV_FILE="$TALARIA_HOME/.env"
    if [ ! -f "$ENV_FILE" ]; then
        return 0
    fi
    ...
    # but opening fails with ENXIO. See #16746.
    if ! (: </dev/tty) 2>/dev/null; then
        log_info "Gateway setup skipped (no terminal available). Run 'talaria gateway install' later."
        return 0
    fi
    ...
    if [ "$should_install_gateway" = true ]; then
        ...
        if [ "$DISTRO" != "termux" ] && command -v systemctl &> /dev/null; then
            ...
            if $TALARIA_CMD gateway install 2>/dev/null; then
                log_success "Gateway service installed"
                if $TALARIA_CMD gateway start 2>/dev/null; then
                    log_success "Gateway started! Your bot is now online."
                else
                    log_warn "Service installed but failed to start. Try: talaria gateway start"
                fi
            else
                log_warn "Systemd install failed. You can start manually: talaria gateway"
            fi
        else
            if [ "$DISTRO" = "termux" ]; then
            ...
            nohup $TALARIA_CMD gateway > "$TALARIA_HOME/logs/gateway.log" 2>&1 &
            GATEWAY_PID=$!
            log_success "Gateway started (PID $GATEWAY_PID). Logs: ~/.talaria/logs/gateway.log"
            log_info "To stop: kill $GATEWAY_PID"
            log_info "To restart later: talaria gateway"
            if [ "$DISTRO" = "termux" ]; then
                log_warn "Android may stop background processes when Termux is suspended or the system reclaims resources."
            fi
        fi
    else
        log_info "Skipped. Start the gateway later with: talaria gateway"
    fi
}
...
print_success() {
    ...
    echo ""
    echo -e "   ${YELLOW}Config:${NC}    $TALARIA_HOME/config.yaml"
    echo -e "   ${YELLOW}API Keys:${NC}  $TALARIA_HOME/.env"
    echo -e "   ${YELLOW}Data:${NC}      $TALARIA_HOME/cron/, sessions/, logs/"
    ...
    echo ""
    echo -e "   ${GREEN}talaria${NC}              Start chatting"
    echo -e "   ${GREEN}talaria setup${NC}        Configure API keys & settings"
    echo -e "   ${GREEN}talaria config${NC}       View/edit configuration"
    echo -e "   ${GREEN}talaria config edit${NC}  Open config in editor"
    echo -e "   ${GREEN}talaria gateway install${NC} Install gateway service (messaging + cron)"
    echo -e "   ${GREEN}talaria update${NC}       Update to latest version"
    echo ""

    echo -e "${CYAN}─────────────────────────────────────────────────────────${NC}"
    echo ""
    if [ "$DISTRO" = "termux" ]; then
        echo -e "${YELLOW}⚡ 'talaria' was linked into $(get_command_link_display_dir), which is already on PATH in Termux.${NC}"
        echo ""
    elif [ "$ROOT_FHS_LAYOUT" = true ]; then
        echo -e "${YELLOW}⚡ 'talaria' was linked into /usr/local/bin and is ready to use — no shell reload needed.${NC}"
        echo ""
    else
        echo -e "${YELLOW}⚡ Reload your shell to use 'talaria' command:${NC}"
        echo ""
        LOGIN_SHELL="$(basename "${SHELL:-/bin/bash}")"
