# Contributing to Talaria Agent

Thank you for contributing to Talaria Agent! This guide covers everything you need: setting up your dev environment, understanding the architecture, deciding what to build, and getting your PR merged.
...
2. **Cross-platform compatibility** — macOS, different Linux distros, and WSL2 on Windows. We want Talaria to work everywhere.
...
Bundled skills (in `skills/`) ship with every Talaria install. They should be **broadly useful to most users**:
...
```bash
mkdir -p ~/.talaria/{cron,sessions,logs,memories,skills}
cp cli-config.yaml.example ~/.talaria/config.yaml
touch ~/.talaria/.env

# Add at minimum an LLM provider key:
echo "OPENROUTER_API_KEY=***" >> ~/.talaria/.env
```
...
mkdir -p ~/.local/bin
ln -sf "$(pwd)/venv/bin/talaria" ~/.local/bin/talaria
...
│
├── talaria_cli/               # CLI command implementations
...
### User configuration (stored in `~/.talaria/`)
...
Four fields are supported under `metadata.talaria`:
...
The user may skip setup and keep loading the skill. Talaria only exposes metadata (`stored_as`, `skipped`, `validated`) to the model — never the secret value.
...
Gateway and messaging sessions never collect secrets in-band; they instruct the user to run `talaria setup` or update `~/.talaria/.env` locally.
...
- **No external dependencies unless absolutely necessary.** Prefer stdlib Python, curl, and existing Talaria tools (`web_extract`, `terminal`, `read_file`).
...
- **Test it.** Run `talaria --toolsets skills -q "Use the X skill to do Y"` and verify the agent follows the instructions correctly.

---

## Adding a Skin / Theme

Talaria uses a data-driven skin system — no code changes needed to add a new skin.

**Option A: User skin (YAML file)**

Create `~/.talaria/skins/<name>.yaml`:
...
Add to `_BUILTIN_SKINS` dict in `talaria_cli/skin_engine.py`. Use the same schema as above but as a Python dict. Built-in skins ship with the package and are always available.

**Activating:**
- CLI: `/skin mytheme` or set `display.skin: mytheme` in config.yaml
- Config: `display: { skin: mytheme }`

See `talaria_cli/skin_engine.py` for the full schema and existing skins as examples.

---

## Cross-Platform Compatibility

Talaria runs on Linux, macOS, and WSL2 on Windows. When writing code that touches the OS:
...
Talaria has terminal access. Security matters.
...
2. **Test manually**: Run `talaria` and exercise the code path you changed
